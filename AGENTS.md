# Tool Usage
Try to use the jetbrains tools provided by Serena.
...
TODOs:
# Implementation Plan: DAP Server

**Branch**: `004-dap-server` | **Date**: 2026-01-07 | **Spec**: `.specify/specs/004-dap-server/spec.md`
**Input**: Feature specification from `/specs/004-dap-server/spec.md`

## Summary

实现完整 DAP Server，MVP 聚焦 JVM 调试，通过 stdio + TCP/UDS 传输为 VSCode 与未来 Python SDK 提供 DAP 会话；复用现有 JSON-RPC 传输与消息处理基础设施，并分阶段扩展 Kotlin/Python。

## Technical Context

**Language/Version**: Kotlin (IntelliJ Platform SDK)
**Primary Dependencies**: IntelliJ Platform SDK, Kotlin, 现有 JSON 序列化 (LspGson)
**Storage**: N/A
**Testing**: 现有项目测试框架（待确认）
**Target Platform**: IntelliJ 插件（桌面 IDE）
**Project Type**: 单体插件工程
**Performance Goals**: 断点命中到停止事件 p95 < 200ms；stackTrace p95 < 500ms
**Constraints**: VSCode stdio 兼容；支持 TCP/UDS 复用；需明确会话状态机
**Scale/Scope**: 单项目多会话、跨语言阶段性扩展

## Constitution Check

- Protocol Conformance: DAP 消息格式与时序必须严格遵循规范。
- Platform Integration First: 复用 IntelliJ Debugger API，避免重复实现调试内核。
- KISS/YAGNI: MVP 仅实现必要 DAP 命令集，避免一次性全量。
...
├── protocol/           # 复用 MessageReader/Writer/JsonRpcHandler
├── server/             # 复用 TCP/UDS server
├── dap/                # 新增 DAP 协议与会话层（规划）
├── dap/handlers/       # DAP 请求/事件处理
├── dap/model/          # DAP 请求/响应/事件模型
└── dap/backend/        # IntelliJ Debugger 适配层
```

**Structure Decision**: 在现有 LSP 架构下新增 `dap/` 子域，复用 `protocol/` 与 `server/`，以避免重复传输实现。
...
### Phase 0: 研究与对齐
- 梳理 DAP MVP 请求/事件集合与 VSCode 默认行为。
- 明确 IntelliJ Debugger API 对接边界。

### Phase 1: 传输与协议层
- 复用 `MessageReader/Writer`，确认 DAP 语义与 LSP 的差异。
- 定义 DAP 消息模型与解析/序列化策略。

### Phase 2: 会话与状态机
- 定义 DAP Session 生命周期与状态转换。
- 设计 DebugSession、断点与线程/栈/变量缓存策略。

### Phase 3: MVP 请求/事件
- 实现 initialize/launch/attach/configurationDone/terminate/ disconnect。
- 实现 setBreakpoints/threads/stackTrace/scopes/variables/evaluate。
- 实现 continue/next/stepIn/stepOut/pause。
- 事件：initialized/stopped/continued/terminated/exited/output/thread。

### Phase 4: VSCode 兼容性与 SDK 契约
- VSCode 集成验证，确认 stdio 行为一致。
- DAPClient 对象 API 与 send/recv 字节语义冻结。

### Phase 5: 扩展规划
- Kotlin 调试扩展（jvm-debugger）。
- Python 调试扩展（python/debugger）。

## Verification Plan

- 手工：VSCode 启动 Java 调试，断点、单步、栈与变量检查。
- 手工：TCP/UDS 模式下启动调试会话。
- 回归：断点命中与 step 事件时序验证。

## Open Questions

- 现有测试框架与自动化能力如何接入 DAP？
- LSP 与 DAP 是否共享端口与连接管理？
- Python SDK 的对象 API 形态（同步/异步）是否提前冻结？

# Feature Specification: DAP Server

**Feature Branch**: `004-dap-server`
**Created**: 2026-01-07
**Status**: Draft
**Input**: 用户需求：实现完整 DAP Server，面向 VSCode 与未来 Python SDK；数据流为 Debug Adapter ↔ IO Handler ↔ DAPClient ↔ Application；其他内容可参考 reference-code/intellij-community

## 背景与目标

- 在 IntelliJ 内实现一个完整的 Debug Adapter Protocol (DAP) Server，复用现有传输/消息基础设施并对接 IntelliJ Debugger。
- 主要消费端为 VSCode Debugger 与未来的 Python SDK（对象 API 驱动）。
- 采用分阶段交付：MVP 聚焦 JVM 调试；后续扩展 Kotlin 和 Python。

## 目标与非目标

### MVP 目标（Phase 1）
- 支持 VSCode 通过 DAP 完成 JVM Debug 会话的核心流程：初始化、断点、运行/单步、栈帧与变量查看、停止/继续、会话结束。
- 支持 stdio + TCP/UDS 传输，具备统一的字节流/对象流处理链路。
- 提供 DAPClient（SDK 侧）所需的对象 API 契约与数据流语义。

### Phase 2 目标
- 在 JVM 能力稳定后扩展 Kotlin 调试支持（参考 `reference-code/intellij-community/plugins/kotlin/jvm-debugger`）。

### Phase 3 目标
- 在 SDK 接口稳定后扩展 Python 调试支持（参考 `reference-code/intellij-community/python/src/com/jetbrains/python/debugger`）。

### 非目标
- 不在本 PRD 中定义具体实现细节（类名、代码结构、线程模型的具体实现方式）。
- 不覆盖 DAP 全量命令的具体实现细节，仅定义阶段范围与必须支持的协议集合。
- 不要求在 MVP 中实现高级功能（数据断点、反汇编、内存读写、性能/进度事件等）。
...
- 发送：应用调用 `client.step_in(threadId=1)`，DAPClient 更新内部状态并将 DAP 消息入队；调用 `client.send()` 获取字节并写入传输通道。
- 接收：传输通道读取字节后调用 `client.recv(bytes)`，DAPClient 解析缓冲区并生成 Event / Response / Request 对象。
- 传输通道：必须支持 stdio 与 TCP/UDS；std.io 用于 VSCode 默认模式，TCP/UDS 复用现有 LSP 服务器能力。
- 消息格式：遵循 DAP 的 `Content-Length` 头部帧格式（与 LSP 类似但语义不同）。

## User Scenarios & Testing *(mandatory)*

### User Story 1 - VSCode 基础调试会话 (Priority: P1)

用户在 VSCode 中启动 Java 调试配置，DAP Server 正常完成 initialize/launch 流程并进入可调试状态。

**Why this priority**: 这是 MVP 必需路径，没有会话建立即无法提供任何调试能力。

**Independent Test**: 使用 VSCode 启动简单 Java 项目调试，观察 DAP 握手与初始化完成。

**Acceptance Scenarios**:
1. **Given** VSCode 发起 `initialize`, **When** Server 返回 capabilities, **Then** 客户端进入配置阶段并收到 `initialized` 事件
2. **Given** 用户配置 `launch`, **When** 发送 `launch` 请求, **Then** Debug 会话进入运行/可断点状态

---

### User Story 2 - 断点与单步 (Priority: P1)

用户设置断点并进行单步操作，DAP Server 能正确发出 stopped/continued 事件并返回线程与栈信息。

**Why this priority**: 断点与单步是调试的核心能力。

**Independent Test**: 在 Java 代码中设置断点，执行 continue/stepIn/stepOut/next，验证 stopped 与 stackTrace。

**Acceptance Scenarios**:
1. **Given** 客户端发送 `setBreakpoints`, **When** 程序命中断点, **Then** Server 发送 `stopped` (reason=breakpoint)
2. **Given** `stopped` 状态, **When** 客户端发送 `next`, **Then** Server 发送 `stopped` (reason=step) 并更新栈帧

---

### User Story 3 - 变量与表达式求值 (Priority: P1)

用户在暂停状态查看变量、作用域与表达式，DAP Server 返回可视化的数据结构。

**Why this priority**: 变量与表达式求值直接决定调试效率。

**Independent Test**: 在断点处请求 `scopes`/`variables`/`evaluate`，验证返回值与展开层级。

**Acceptance Scenarios**:
1. **Given** `stopped` 状态, **When** 发送 `scopes`, **Then** 返回 Arguments/Locals 等作用域
2. **Given** 作用域变量 reference, **When** 发送 `variables`, **Then** 返回变量列表与可展开引用

---

### User Story 4 - Python SDK 对象 API（契约） (Priority: P2)

未来 Python SDK 以对象 API 驱动 DAPClient，通过 send/recv 对接字节流。

**Why this priority**: SDK 是后续生态扩展的关键，PRD 需明确契约边界。

**Independent Test**: 使用模拟传输注入 DAP 字节流，验证 DAPClient 生成的对象与状态转移。

**Acceptance Scenarios**:
1. **Given** DAPClient 收到 `stopped` 事件字节流, **When** 调用 `recv`, **Then** 返回 `StoppedEvent` 对象
2. **Given** 应用调用 `client.continue(threadId=1)`, **When** 调用 `send`, **Then** 返回待发送字节序列

---

### Edge Cases

- **未初始化请求**：未完成 `initialize` 前收到请求，必须返回协议错误。
- **断连与重连**：传输断开后允许重新建立会话，不污染旧 session。
- **线程/栈为空**：目标进程无可用线程或栈帧时返回空数组并保持协议合法。
- **断点无效**：断点无法绑定时返回 `verified=false` 并附带说明。
- **长输出**：大量 `output` 事件需要限速或分片，避免阻塞主循环。

## Requirements *(mandatory)*

### Functional Requirements

- **FR-001**: Server MUST 支持 DAP `Content-Length` 帧格式的收发与解析。
- **FR-002**: Server MUST 支持 stdio 传输；SHOULD 支持 TCP/UDS 传输。
- **FR-003**: Server MUST 实现 DAP 会话状态机（initialize → initialized → configuration → running）。
- **FR-004**: Server MUST 支持 MVP 请求集：`initialize`, `launch`, `attach`, `configurationDone`, `disconnect`, `terminate`。
- **FR-005**: Server MUST 支持调试控制请求：`setBreakpoints`, `threads`, `stackTrace`, `scopes`, `variables`。
- **FR-006**: Server MUST 支持运行控制请求：`continue`, `next`, `stepIn`, `stepOut`, `pause`。
- **FR-007**: Server MUST 支持表达式求值：`evaluate`。
- **FR-008**: Server MUST 支持事件：`initialized`, `stopped`, `continued`, `terminated`, `exited`, `output`, `thread`。
- **FR-009**: Server MUST 将 DAP 请求映射到 IntelliJ Debugger API（JVM），并保证线程/栈/变量一致性。
- **FR-010**: Server MUST 返回符合 DAP 定义的 `success=false` 错误响应并包含可诊断信息。
- **FR-011**: Server SHOULD 支持 `setExceptionBreakpoints`（至少支持默认异常过滤器）。
- **FR-012**: DAPClient MUST 提供对象 API 与 `send/recv` 字节管道语义。

### Non-Functional Requirements

- **NFR-001**: 断点命中到 `stopped` 事件的 95th 延迟 < 200ms（本地调试）。
- **NFR-002**: `stackTrace` 响应 95th 延迟 < 500ms（<=50 frames）。
- **NFR-003**: 高并发请求必须被串行化或排队，避免状态竞争。
- **NFR-004**: 日志需区分协议错误、调试后端错误与传输错误。
- **NFR-005**: 兼容 VSCode DAP 客户端的默认行为（stdio、初始化能力协商）。

### Key Entities

- **DapServer**: DAP Server 实例，管理连接、会话与请求路由。
- **DapTransport**: 传输抽象（stdio / TCP / UDS）。
- **IoHandler**: 字节流读写与帧解析。
- **DapClient**: SDK 侧对象 API，负责请求/事件对象化。
- **DebugSession**: 调试会话状态机与上下文（线程、断点、栈）。
- **DebuggerBackend**: IntelliJ Debugger API 适配层。

## Success Criteria *(mandatory)*

### Measurable Outcomes

- **SC-001**: VSCode 能完成 `initialize` → `initialized` → `launch` 全流程，并进入调试状态。
- **SC-002**: 断点命中后 1s 内可获取 `threads` 与 `stackTrace` 正确结果。
- **SC-003**: 单步操作（next/stepIn/stepOut）后 `stopped` 事件正确触发。
- **SC-004**: 变量与表达式求值在暂停状态下正确返回。
- **SC-005**: stdio 与 TCP/UDS 传输均可完成完整调试流程。

## 参考与依据

- `debugAdapterProtocol.json`
- `src/main/kotlin/com/frenchef/intellijlsp/protocol/JsonRpcHandler.kt`
- `src/main/kotlin/com/frenchef/intellijlsp/protocol/MessageReader.kt`
- `src/main/kotlin/com/frenchef/intellijlsp/protocol/MessageWriter.kt`
- `src/main/kotlin/com/frenchef/intellijlsp/protocol/PendingRequestManager.kt`
- `src/main/kotlin/com/frenchef/intellijlsp/protocol/LspGson.kt`
- `src/main/kotlin/com/frenchef/intellijlsp/server/TcpLspServer.kt`
- `src/main/kotlin/com/frenchef/intellijlsp/server/UdsLspServer.kt`
- `src/main/kotlin/com/frenchef/intellijlsp/server/LspServerManager.kt`
- `reference-code/intellij-community/java/debugger`
- `reference-code/intellij-community/plugins/kotlin/jvm-debugger`
- `reference-code/intellij-community/python/src/com/jetbrains/python/debugger`

## 风险与未决问题

- **DAP 与 JSON-RPC 差异**：需明确协议语义差异，避免混用 LSP 处理逻辑。
- **线程模型**：调试事件与请求的并发顺序需要明确（暂停/继续互斥）。
- **路径映射**：VSCode 与 IntelliJ 的源路径/URI 规范需定义转换策略。
- **异常策略**：未绑定断点、后端异常与取消请求的错误语义需进一步确认。
- **Python SDK**：对象 API 形态与错误模型需与 SDK 设计协商后冻结。

---
description: "Tasks for DAP Server implementation"
---

# Tasks: DAP Server

**Input**: Design documents from `/specs/004-dap-server/`
...
- [ ] T001 Create DAP spec scaffolding in `.specify/specs/004-dap-server/` (done: `spec.md`, `plan.md`)
- [ ] T002 Define DAP module package layout in `src/main/kotlin/com/frenchef/intellijlsp/dap/`

---

## Phase 2: Foundational (Blocking Prerequisites)

**Purpose**: DAP 传输、消息模型、会话基础设施

- [ ] T010 [P] [US1] Define DAP message models in `src/main/kotlin/com/frenchef/intellijlsp/dap/model/`
- [ ] T011 [P] [US1] Implement DAP framing adapter (Content-Length) reusing `protocol/MessageReader.kt` and `protocol/MessageWriter.kt`
- [ ] T012 [US1] Implement DAP session state machine in `src/main/kotlin/com/frenchef/intellijlsp/dap/`
- [ ] T013 [P] [US1] Implement DAP request router in `src/main/kotlin/com/frenchef/intellijlsp/dap/handlers/`
- [ ] T014 [US1] Implement IntelliJ Debugger backend adapter in `src/main/kotlin/com/frenchef/intellijlsp/dap/backend/`
- [ ] T015 [US1] Implement error mapping & logging conventions in `src/main/kotlin/com/frenchef/intellijlsp/dap/`

---

## Phase 3: User Story 1 - VSCode 基础调试会话 (Priority: P1)

**Goal**: initialize/launch 会话建立与 DAP 基础事件

**Independent Test**: VSCode 启动 Java 调试并完成 initialize/launch

- [ ] T020 [US1] Implement `initialize` handling and capabilities response in `dap/handlers/`
- [ ] T021 [US1] Implement `initialized` event emission in `dap/handlers/`
- [ ] T022 [US1] Implement `launch`/`attach` request handling in `dap/handlers/`
- [ ] T023 [US1] Implement `configurationDone` request handling in `dap/handlers/`
- [ ] T024 [US1] Implement `disconnect`/`terminate` handling in `dap/handlers/`

---

## Phase 4: User Story 2 - 断点与单步 (Priority: P1)

**Goal**: setBreakpoints + continue/step + stopped 事件链路

**Independent Test**: 设置断点，单步执行并正确产生 stopped 事件

- [ ] T030 [US2] Implement `setBreakpoints` in `dap/handlers/` and map to backend adapter
- [ ] T031 [US2] Implement `threads`, `stackTrace` in `dap/handlers/`
- [ ] T032 [US2] Implement `continue`, `next`, `stepIn`, `stepOut`, `pause` in `dap/handlers/`
- [ ] T033 [US2] Implement events `stopped`, `continued`, `thread` in `dap/handlers/`

---

## Phase 5: User Story 3 - 变量与表达式求值 (Priority: P1)

**Goal**: scopes/variables/evaluate 全链路

**Independent Test**: 断点处获取 scopes/variables，并能 evaluate 表达式

- [ ] T040 [US3] Implement `scopes` in `dap/handlers/`
- [ ] T041 [US3] Implement `variables` in `dap/handlers/`
- [ ] T042 [US3] Implement `evaluate` in `dap/handlers/`

---

## Phase 6: User Story 4 - Python SDK 对象 API（契约） (Priority: P2)

**Goal**: 明确 DAPClient send/recv 语义与对象模型

**Independent Test**: mock 传输注入字节流，确认对象解析与状态转移

- [ ] T050 [US4] Define DAPClient API contract in `dap/` (interfaces only)
- [ ] T051 [US4] Define Event/Response/Request DTO mapping rules in `dap/model/`
- [ ] T052 [US4] Document send/recv buffering behavior and partial frames

---

## Phase 7: Polish & Cross-Cutting Concerns

- [ ] T060 [P] Add logging & metrics hooks in `dap/`
- [ ] T061 [P] Add VSCode compatibility checks and manual verification steps in `specs/004-dap-server/`
- [ ] T062 [P] Document error codes mapping in `specs/004-dap-server/`
...
- US1 → US2 → US3 顺序推荐，但 US2/US3 可在后端能力具备后并行推进。
- US4 可与 US2/US3 并行（独立于调试后端）。
