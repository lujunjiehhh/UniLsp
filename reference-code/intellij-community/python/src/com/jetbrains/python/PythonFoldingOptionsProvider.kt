...
private val longStringLiterals: CheckboxDescriptor
    get() = CheckboxDescriptor(
        PyBundle.message("python.long.string.literals"),
        PythonFoldingSettings.getInstance()::COLLAPSE_LONG_STRINGS
    )
private val longCollectionLiterals: CheckboxDescriptor
    get() = CheckboxDescriptor(
        PyBundle.message("python.long.collection.literals"),
        PythonFoldingSettings.getInstance()::COLLAPSE_LONG_COLLECTIONS
    )
private val sequentialComments: CheckboxDescriptor
    get() = CheckboxDescriptor(
        PyBundle.message("python.sequential.comments"),
        PythonFoldingSettings.getInstance()::COLLAPSE_SEQUENTIAL_COMMENTS
    )
private val typeAnnotations: CheckboxDescriptor
    get() = CheckboxDescriptor(
        PyBundle.message("python.type.annotations"), PythonFoldingSettings.getInstance()::COLLAPSE_TYPE_ANNOTATIONS,
        comment = PyBundle.message("python.type.annotations.hint")
    )

class PythonFoldingOptionsProvider : UiDslUnnamedConfigurable.Simple(), CodeFoldingOptionsProvider,
