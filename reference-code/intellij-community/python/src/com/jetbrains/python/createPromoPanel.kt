 ...
 */
@RequiresEdt
fun createPromoPanel(@Nls descriptionHtml: String? = null, features: List<PromoFeatureListItem>? = null): JPanel =
    panel {
        row {
                ...
                if (features == null) {
                    row {
                        text(PyBundle.message("pycharm.free.mode.upgrade.body")).align(AlignY.TOP).applyToComponent {
                            putClientProperty(
                                DslComponentProperty.VERTICAL_COMPONENT_GAP,
                                VerticalComponentGap(top = true, bottom = false)
                            )
                        }
                    }.topGap(TopGap.SMALL)
                } else {
