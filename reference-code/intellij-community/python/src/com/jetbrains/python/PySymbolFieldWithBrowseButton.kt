 ...
 *
 */
class PySymbolFieldWithBrowseButton(
    contextAnchor: ContextAnchor,
    filter: ((PsiElement) -> Boolean)? = null,
    startFromDirectory: (() -> VirtualFile)? = null
) : TextAccessor, ComponentWithBrowseButton<TextFieldWithCompletion>(
    TextFieldWithCompletion(
        contextAnchor.project,
        PyNameCompletionProvider(contextAnchor, filter, startFromDirectory),
        "",
        true,
        true,
        true
    ...
    init {
        ...
        putClientProperty(
            DslComponentProperty.VISUAL_PADDINGS,
            UnscaledGaps(3, 3, 3, JBUI.unscale(button.insets.right))
        )
    }
...
private fun PyType.getVariants(element: PsiElement): Array<LookupElement> =
    this.getCompletionVariants("", element, ProcessingContext()).filterIsInstance(LookupElement::class.java)
        .toTypedArray()

private class PyNameCompletionProvider(
    private val contextAnchor: ContextAnchor,
    private val filter: ((PsiElement) -> Boolean)?,
    private val startFromDirectory: (() -> VirtualFile)? = null
    ...
    override fun addCompletionVariants(text: String, offset: Int, prefix: String, result: CompletionResultSet) {
        ...
        if ('.' !in text) {
            ...
            val resolveContext = QNameResolveContext(
                contextAnchor, evalContext = evalContext, allowInaccurateResult = false,
...
private class PySymbolChooserDialog(
    project: Project,
    scope: GlobalSearchScope,
    private val filter: ((PsiElement) -> Boolean)?
) : PyTreeChooserDialog<PsiNamedElement>(
    PyBundle.message("python.symbol.chooser.dialog.title"),
    ...
    override fun createChooseByNameModel(): GotoSymbolModel2 {
        return GotoSymbolModel2(
            project,
            java.util.List.of<ChooseByNameContributor>(object : PyGotoSymbolContributor(), DumbAware {
