...
class PyDataViewDialog(private val project: Project, value: PyDebugValue) : DialogWrapper(project, false) {
    ...
    private fun createJupyterSuggestionPanel(): JPanel? {
        ...
        fun supposeRestartIfFRequired() {
            val requireRestart =
                toInstallOrEnable.firstOrNull { PluginManagerCore.getPlugin(it)?.isRequireRestart == true } != null
            if (requireRestart) {
    ...
    companion object {
        ...
        private const val JUPYTER_SUGGESTION_ENABLED_PROPERTY_KEY =
            "python.debugger.dataView.jupyter.suggestion.enabled"

        private fun isJupyterSuggestionEnabled(project: Project) =
            PropertiesComponent.getInstance(project).getBoolean(JUPYTER_SUGGESTION_ENABLED_PROPERTY_KEY, true)

        private fun disableJupyterSuggestions(project: Project) =
            PropertiesComponent.getInstance(project).setValue(JUPYTER_SUGGESTION_ENABLED_PROPERTY_KEY, false, true)
    }
}