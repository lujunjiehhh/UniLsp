...
class TypeNameCompletionProvider(val project: Project) : TextCompletionProvider {
    ...
    private fun addClassesFromPyFile(
    ...
    private fun addVariantsFromModuleComponents(
        parameters: CompletionParameters,
        result: CompletionResultSet,
        ...
        for (element in getElementsFromModule(moduleComponents, project)) {
            when (element) {
                is PsiDirectory -> {
                    val modules =
                        PyModuleType.getSubModuleVariants(element, parameters.position.parent, alreadyAddedModulesNames)
    ...
    private fun addClassesVariants(parameters: CompletionParameters, result: CompletionResultSet) {
        ...
        for (elementName in result.prefixMatcher.sortMatching(clsKeys)) {
            stubIndex.processElements(
                ...
                val importPath =
                    QualifiedNameFinder.findCanonicalImportPath(element, null) ?: return@processElements true
