...
class PyDataView(private val project: Project) : DumbAware {
    ...
    fun addTab(frameAccessor: PyFrameAccessor): Content {
        ...
        if (frameAccessor is PydevConsoleCommunication) {
            ...
            content.description =
                PyBundle.message("debugger.data.view.connected.to.debug.session", frameAccessor.session.sessionName)
        }
    ...
    companion object {
        ...
        @JvmStatic
        fun isAutoResizeEnabled(project: Project): Boolean =
            PropertiesComponent.getInstance(project).getBoolean(AUTO_RESIZE, true)

        @JvmStatic
        fun isColoringEnabled(project: Project): Boolean =
            PropertiesComponent.getInstance(project).getBoolean(COLORED_BY_DEFAULT, true)
