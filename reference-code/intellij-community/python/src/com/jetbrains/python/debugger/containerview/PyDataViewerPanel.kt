...
class PyDataViewerPanel(
    ...
    fun switchBetweenCommunityAndFactoriesTables() {
        ...
        if (debugValue != null && !isSupportedByCommunityDataViewer(debugValue)) {
            isPanelFromFactory = true
            PyDataViewerPanelHelper.showCommunityDataViewerRestrictionsBalloon(
                tablePanel = component,
                tableParentPanel = this
            )
            return
        }
    ...
    companion object {
        ...
        private const val DATA_VIEWER_SHOW_COMMUNITY_PANEL =
            "py.data.viewer.powerful.table.shown.on.each.data.view.opening"

        fun isUsingCommunityPanel(): Boolean =
            PropertiesComponent.getInstance().getBoolean(DATA_VIEWER_SHOW_COMMUNITY_PANEL, true)

        fun setUsingCommunityPanel(value: Boolean): Unit =
            PropertiesComponent.getInstance().setValue(DATA_VIEWER_SHOW_COMMUNITY_PANEL, value, true)
    }
}