...
private fun loadCustomChildren(
...
fun loadTypeRendererChildren(
    frameAccessor: PyFrameAccessor,
    debugValue: PyDebugValue,
    typeRenderer: PyUserNodeRenderer
): XValueChildrenList {
    ...
    companion object {
        fun showSettingsWithNewRenderer(project: Project, debugValue: PyDebugValue) {
            ShowSettingsUtil.getInstance().showSettingsDialog(project, PyUserTypeRenderersConfigurable::class.java) {
                ...
                newRenderer.name =
                    debugValue.type + " " + PyBundle.message("form.debugger.variables.view.user.type.renderers.renderer")
                ...
                if (pyClass != null) {
                    ...
                    val classes =
                        PyClassNameIndex.find(debugValue.type, project, GlobalSearchScope.projectScope(project))
...
class ConfigureTypeRenderersHyperLink(
    ...
    private val TEXT_ATTRIBUTES =
        SimpleTextAttributes(SimpleTextAttributes.STYLE_PLAIN, JBUI.CurrentTheme.Link.Foreground.PRESSED)
