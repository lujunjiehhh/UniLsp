...
internal object PyDataViewerPanelHelper {
    fun showCommunityDataViewerRestrictionsBalloon(
        tablePanel: PyDataViewerAbstractPanel,
        tableParentPanel: PyDataViewerPanel
    ) {
        createBalloon(tablePanel)
            .show(
                RelativePoint(
                    tableParentPanel,
                    Point(
                        tableParentPanel.width - (tablePanel.topToolbar?.preferredWidth ?: 10) / 2,
                        tablePanel.topToolbar?.preferredHeight ?: 0
                    )
                ),
    ...
    private fun createBalloon(component: PyDataViewerAbstractPanel): Balloon {
        return JBPopupFactory.getInstance()
            .createHtmlTextBalloonBuilder(
                PyBundle.message("debugger.dataViewer.dataframes.unsupported"),
                MessageType.INFO,
                null
            ).apply {
