...
class PyUserTypeRenderersConfigurable : SearchableConfigurable {
    ...
    override fun getDisplayName(): String =
        PyBundle.message("configurable.PyUserTypeRenderersConfigurable.display.name")
    ...
    private fun setupRendererChooser() {
        myRendererChooser.emptyText.text =
            PyBundle.message("form.debugger.variables.view.user.type.renderers.no.renderers")
    ...
    private inner class RendererSettings : JPanel(BorderLayout()), Disposable {
        ...
        private val myTypeNameTextField = TextFieldWithCompletion(
            myProject, TypeNameCompletionProvider(myProject), "",
            true, true, true
        )
        private val myNodeValueExpressionEditor = XDebuggerExpressionEditor(
            myProject, PyDebuggerEditorsProvider(), "NodeValueExpression", null,
            ...
            return panel {
                ...
                row {
                    cell(myTypeNameTextField)
                        .label(
                            PyBundle.message("form.debugger.variables.view.user.type.renderers.apply.renderer.to.objects.of.type"),
                            LabelPosition.TOP
                        )
                        .align(AlignX.FILL)
                }
            ...
            val myTypeNameFieldValidator = ComponentValidator(this).withValidator(
                Supplier<ValidationInfo?> {
                    ...
                    return@Supplier if (!isValidTypeName(text)) {
                        ValidationInfo(
                            PyBundle.message("form.debugger.variables.view.user.type.renderers.class.not.found"),
                            myTypeNameTextField
                        )
                    } else {
                        null
        ...
        private fun createChildrenListEditor(): JComponent {
            ...
            return ChildrenListEditorToolbarDecorator(
                myChildrenListEditorTable,
                myChildrenListEditorTableModel
        ...
        private fun resetTypeInfo(renderer: PyUserNodeRenderer) {
            ...
            renderer.moduleRootHasOneTypeWithSameName = getClassesNumberInModuleRootWithName(
                cls, importPath ?: QualifiedName.fromComponents(),
                myProject
            ) == 1
        }
        ...
        private inner class ChildrenListEditorToolbarDecorator(
            val table: JBTable,
            val tableModel: ChildrenListEditorTableModel
        ) {
