    ...
    private inner class SurroundWithCastWorker(
        ...
        override fun typeCalculationFinished(type: KotlinType?) {
            ...
            DebuggerInvocationUtil.invokeLater(project, Runnable {
                writeCommandAction(project).withName(JavaDebuggerBundle.message("command.name.surround.with.runtime.cast"))
                    .run<Nothing> {
                        try {
                            ...
                            val parentCast =
                                factory.createExpression("(expr as " + fqName.asString() + ")") as KtParenthesizedExpression
