...
internal class CodeFragmentParameterAnalyzer(
        ...
        return parameters.getOrPut(descriptor) {
            ...
            SmartCodeFragmentParameter(
                Dumb(
                    ...
                    "",
                    depthRelativeToCurrentFrame = 0,
                    AsmUtil.THIS + debugLabel
                ), type, descriptor
            )
        }
    }

    private fun processExtensionReceiver(
    ...
    private fun processContextReceiver(
        ...
        return parameters.getOrPut(receiverParameter) {
            ...
            SmartCodeFragmentParameter(
                Dumb(Kind.CONTEXT_RECEIVER, name, depthRelativeToCurrentFrame = 0, label),
                receiverType,
                receiverParameter
            )
        }
    ...
    private fun processSyntheticFieldVariable(descriptor: SyntheticFieldDescriptor): SmartCodeFragmentParameter {
        ...
        return parameters.getOrPut(descriptor) {
            SmartCodeFragmentParameter(
                Dumb(Kind.FIELD_VAR, fieldName, depthRelativeToCurrentFrame = 0, "field"),
                type,
                descriptor
            )
        }
    ...
    private fun processSimpleNameExpression(
        ...
        return when (target) {
            ...
            is ValueDescriptor -> {
                ...
                parameters.getOrPut(target) {
                    val kind =
                        if (target is LocalVariableDescriptor && target.isDelegated) Kind.DELEGATED else Kind.ORDINARY
                    SmartCodeFragmentParameter(
                        Dumb(kind, target.name.asString(), depthRelativeToCurrentFrame = 0),
                        target.type,
                        target,
                        isLValue
                    )
    ...
    private fun isAssignmentLValue(expression: PsiElement): Boolean {
        val assignmentExpression =
            (expression.parent as? KtBinaryExpression)?.takeIf { KtPsiUtil.isAssignment(it) } ?: return false
    ...
    private fun processCoroutineContextCall(target: DeclarationDescriptor): SmartCodeFragmentParameter? {
        if (target is PropertyDescriptor && target.fqNameSafe == COROUTINE_CONTEXT_FQ_NAME) {
            return parameters.getOrPut(target) {
                SmartCodeFragmentParameter(
                    Dumb(Kind.COROUTINE_CONTEXT, "", depthRelativeToCurrentFrame = 0),
                    target.type,
                    target
                )
            }
    ...
    fun checkBounds(
        ...
        parameter: SmartCodeFragmentParameter?
    ) {
    ...
    private fun doesCrossInlineBounds(expression: PsiElement, declaration: PsiElement): Boolean {
        ...
        fun findCommonParent(declaration: PsiElement): PsiElement? {
            ...
            return when {
                declarationParent is KtDestructuringDeclaration -> declarationParent.getParentOfType<KtBlockExpression>(
                    true
                )

                declarationParent is KtParameterList && declarationParent.parent is KtPrimaryConstructor -> declaration.getParentOfType<KtClass>(
                    true
                )
        ...
        var currentParent: PsiElement? =
            expression.parent?.takeIf { it.isInside(declarationCommonParent) } ?: return false
