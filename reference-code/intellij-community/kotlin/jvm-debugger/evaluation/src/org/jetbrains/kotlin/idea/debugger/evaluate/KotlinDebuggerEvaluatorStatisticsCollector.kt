...
object KotlinDebuggerEvaluatorStatisticsCollector : CounterUsagesCollector() {
    ...
    private val stdInvocationExceptionField =
        EventFields.StringValidatedByCustomRule("invocation_exception", StandardExceptionNameRuleValidator::class.java)

    // events
    private val analysisCompilationEvent = GROUP.registerVarargEvent(
        "analysis.compilation.result",
        compilerField,
        originField,
        compilationResultField,
        wrapTimeMsField,
        analysisTimeMsField,
        compilationTimeMsField,
        wholeTimeField,
        interruptionsField,
        compilerExceptionField
    )

    // no need to record evaluation time, as it reflects what user evaluates, not how effective our evaluation is
    private val evaluationEvent = GROUP.registerVarargEvent(
        "evaluation.result",
        resultField,
        compilerField,
        originField,
        stdInvocationExceptionField
    )
