...
class K2KotlinCodeFragmentCompiler : KotlinCodeFragmentCompiler {
    ...
    override fun compileCodeFragment(
        ...
        try {
            ...
            onFinish(
                if (isJustInvalidUserCode) EvaluationCompilerResult.COMPILATION_FAILURE
    ...
    private fun unwrapEvaluationException(e: Throwable): Throwable {
        var current = e
        while (true) {
            val next = (current as? KaRestrictedAnalysisException)?.cause ?: (current as? ExecutionException)?.cause
            ?: return current
            current = next
        }
    }
    ...
    private class ExecutionStack(val context: ExecutionContext) : Sequence<PsiElement?> {
        override fun iterator(): Iterator<PsiElement?> = object : Iterator<PsiElement?> {
            ...
            private val frames by lazy {
                context.frameProxy.stackFrame.computeKotlinStackFrameInfos(
                    alwaysComputeCallLocations = true
                )
            }
    ...
    @OptIn(KaExperimentalApi::class)
    private fun compiledCodeFragmentDataK2Impl(
        context: ExecutionContext,
        codeFragment: KtCodeFragment
    ): CompiledCodeFragmentData {
            ...
            try {
                ...
                when (val result =
                    ...
                    is KaCompilationResult.Success -> {
                        ...
                        val ideCompilationResult =
                            CompilationResult(classes, parameterInfo, mapOf(), methodSignature, CompilerType.K2)
    ...
    private fun KaCodeFragmentCapturedValue.toDumbCodeFragmentParameter(): CodeFragmentParameter.Dumb? = when (this) {
        ...
        is KaCodeFragmentCapturedValue.ContainingClass ->
            CodeFragmentParameter.Dumb(
                CodeFragmentParameter.Kind.DISPATCH_RECEIVER,
                "",
        ...
        is KaCodeFragmentCapturedValue.ExtensionReceiver ->
            CodeFragmentParameter.Dumb(
                CodeFragmentParameter.Kind.EXTENSION_RECEIVER,
                name,
                depthRelativeToCurrentFrame,
                displayText
            )
        ...
        is KaCodeFragmentCapturedValue.ContextReceiver -> {
            ...
            CodeFragmentParameter.Dumb(
                CodeFragmentParameter.Kind.CONTEXT_RECEIVER,
                name,
                depthRelativeToCurrentFrame,
                displayText
            )
        ...
        is KaCodeFragmentCapturedValue.BackingField ->
            CodeFragmentParameter.Dumb(
                CodeFragmentParameter.Kind.FIELD_VAR,
                name,
                depthRelativeToCurrentFrame,
                displayText
            )
    ...
    runReadAction {
        codeFragment.accept(object : PsiRecursiveElementVisitor() {
            override fun visitElement(element: PsiElement) {
                if (result) return
                result =
                    element is KtOperationReferenceExpression && element.operationSignTokenType == KtTokens.AS_KEYWORD
