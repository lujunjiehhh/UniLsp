...
class KotlinEvaluator(val codeFragment: KtCodeFragment, private val sourcePosition: SourcePosition?) : Evaluator,
    ...
    override fun evaluate(context: EvaluationContextImpl): Any? {
        ...
        try {
            ...
            reportError(
                codeFragment,
                sourcePosition,
                e.message ?: KotlinDebuggerEvaluationBundle.message("error.exception.occurred"),
                e
            )
                ...
                try {
                    evaluateWithEval4J(context, compiledData, classLoaderRef).also {
                        reportErrorWithAttachments(
                            ...
                            codeFragment,
                            original,
                            headerMessage = "Eval4J success, but compiling evaluator failed with: "
                        )
                    }
                } catch (e: Throwable) {
    ...
    private class OnRefreshCachedValueProvider(private val project: Project) :
        CachedValueProvider<ConcurrentFactoryCache<String, CompilationCodeFragmentResult>> {
        override fun compute(): CachedValueProvider.Result<ConcurrentFactoryCache<String, CompilationCodeFragmentResult>> {
            ...
            return CachedValueProvider.Result(
                ...
                KotlinDebuggerSessionRefreshTracker.getInstance(project)
            )
        }
    }
    ...
    private fun evaluateWithEval4J(
        ...
        return runEvaluation(context, compiledData, classLoader ?: context.evaluationContext.classLoader) { args ->
            ...
            val eval = object : JDIEval(vm, classLoader, thread, context.invokePolicy) {
                override fun jdiInvokeStaticMethod(
                    type: ClassType,
                    method: Method,
                    args: List<Value?>,
                    invokePolicy: Int
            ...
            if (mayRetry) {
                DebuggerUtils.getInstance()
                    .processCollectibleValue(object : ThrowableComputable<InterpreterResult, EvaluateException> {
    ...
    private fun updateLocalVariableValue(converter: EvaluatorValueConverter, ref: VariableFinder.RefWrapper) {
        ...
        if (variable != null) {
            try {
                ...
                LOG.error(
                    "Cannot update local variable value: expected type ${variable.type}, actual type ${newValue?.type()}",
                    e
                )
            }
        ...
        return args.map { (parameter, asmType) ->
            ...
            if (result == null) {
                ...
                fun isInsideDefaultInterfaceMethod(): Boolean {
                    ...
                    return method.name()
                        .endsWith("\$default") && DEFAULT_METHOD_MARKERS.any { desc.contains("IL$it;)") }
                }
