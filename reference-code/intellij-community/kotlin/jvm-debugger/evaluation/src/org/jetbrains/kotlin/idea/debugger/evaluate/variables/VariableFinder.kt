...
internal class VariableFinder(val context: ExecutionContext) {
    ...
    sealed class VariableKind(val asmType: AsmType) {
        ...
        class ExtensionThis(val label: String, asmType: AsmType) : VariableKind(asmType) {
            val parameterName =
                getLabeledThisName(label, AsmUtil.LABELED_THIS_PARAMETER, AsmUtil.RECEIVER_PARAMETER_NAME)
            val fieldName = getLabeledThisName(
                label,
                getCapturedFieldName(AsmUtil.LABELED_THIS_FIELD),
    ...
    fun find(parameter: CodeFragmentParameter, asmType: AsmType, codeFragment: KtCodeFragment): Result? {
        return when (parameter.kind) {
            ...
            Kind.DISPATCH_RECEIVER -> findDispatchThis(
                VariableKind.OuterClassThis(asmType),
                parameter.depthRelativeToCurrentFrame
            )
    ...
    private fun findExtensionThis(kind: VariableKind.ExtensionThis, rollbackInlineFramesCount: Int): Result? {
        ...
        findLocalVariableWithSpecifiedRollbackFramesCount(
            variables,
            kind,
            rollbackInlineFramesCount,
            namePredicate
        )?.let { return it }
    ...
    private fun findDispatchThis(kind: VariableKind.OuterClassThis, rollbackInlineFramesCount: Int): Result? {
        ...
        if (inlineDepth > 0) {
            variables.namedEntitySequence()
                .filter {
                    it.name.matches(INLINED_THIS_REGEX) && getInlineDepth(it.name) == inlineDepth && kind.typeMatches(
                        it.type
                    )
                }
                .mapNotNull { it.unwrapAndCheck(kind) }
    ...
    private fun findLocalVariable(
        ...
        return findLocalVariableWithSpecifiedRollbackFramesCount(
            variables,
            kind,
            rollbackInlineFramesCount
        ) { it == name }
    ...
    private fun findCoroutineContextForContinuation(continuation: ObjectReference): ObjectReference? {
        ...
        val getContextMethod =
            DebuggerUtils.findMethod(continuationType, "getContext", "()Lkotlin/coroutines/CoroutineContext;")
                ?: return null
