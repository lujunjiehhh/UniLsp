...
private class K2DfaAssistProvider : DfaAssistProvider {
                    ...
                    is VariableResult.Variable -> {
                        ...
                        if (value != null) {
                            if (inlineDepth > 0 && value.type() is PrimitiveType && !result.isNonNullPrimitiveReturnType) {
                                ...
                                if (typeKind != null) {
                                    val referenceType =
                                        proxy.virtualMachine.classesByName(typeKind.boxedFqn).firstOrNull()
            ...
            if (descriptor is KtVariableDescriptor) {
                ...
                when (result) {
                    QualifierVariableResult.InlineClassProperty -> {
                        map[descriptor] =
                            if (qualifier is DfaAssistProvider.InlinedValue) qualifier.value else qualifier
                    }
    ...
    private sealed interface VariableResult {
        ...
        data class Variable(val name: String, val psi: PsiElement?, val isNonNullPrimitiveReturnType: Boolean) :
            VariableResult
    }
