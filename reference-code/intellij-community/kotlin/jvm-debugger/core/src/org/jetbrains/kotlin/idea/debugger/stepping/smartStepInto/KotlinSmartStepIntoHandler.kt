...
class KotlinSmartStepIntoHandler : JvmSmartStepIntoHandler() {
    ...
    override fun findStepIntoTargets(
        position: SourcePosition,
        session: DebuggerSession
    ): Promise<List<SmartStepTarget>> =
    ...
    override fun findSmartStepTargetsAsync(
        position: SourcePosition,
        session: DebuggerSession
    ): Promise<List<SmartStepTarget>> {
        ...
        return session.process.managerThread.coroutineScope.async {
            withDebugContext(debuggerContext) {
                val suspendContext =
                    (DebuggerManagerThreadImpl.getCurrentCommand() as SuspendContextCommandImpl).suspendContext
    ...
    override fun findSmartStepTargets(position: SourcePosition): List<SmartStepTarget> =
        findSmartStepTargetsSync(position, null, null)
    ...
    private suspend fun findSmartStepTargets(
        position: SourcePosition,
        session: DebuggerSession?,
        suspendContext: SuspendContextImpl?
    ): List<SmartStepTarget> {
        ...
        if (session != null && location != null) {
            ...
            if (currentMethodName?.endsWith("\$default") == true) {
                DebuggerStatistics.logSmartStepIntoTargetsDetection(
                    session.project,
                    Engine.KOTLIN,
                    SmartStepIntoDetectionStatus.NO_TARGETS
                )
                return emptyList()
            }
...
private suspend fun calculateSmartStepTargetsToShow(
...
suspend fun List<KotlinMethodSmartStepTarget>.filterAlreadyExecuted(
    ...
    if (isEmpty()) {
        DebuggerStatistics.logSmartStepIntoTargetsDetection(
            ...
            Engine.KOTLIN,
            SmartStepIntoDetectionStatus.SUCCESS
        )
        return this
    }
...
private suspend fun filterSmartStepTargets(
    ...
    var (unvisitedTargets, unvisitedAtTheEnd) = targetFiltererAdapter.visitTrace(
        targetFilterer,
    ...
    if (unvisitedAtTheEnd.isNotEmpty()) {
        ...
        if (Registry.`is`("debugger.kotlin.report.smart.step.into.targets.detection.failure")) {
            JvmSmartStepIntoErrorReporter.report(
                expression,
                session,
                position,
                "Failed to locate target calls: $targetStrings"
            )
        }
        DebuggerStatistics.logSmartStepIntoTargetsDetection(
            session?.project,
            Engine.KOTLIN,
            SmartStepIntoDetectionStatus.TARGETS_MISMATCH
        )
