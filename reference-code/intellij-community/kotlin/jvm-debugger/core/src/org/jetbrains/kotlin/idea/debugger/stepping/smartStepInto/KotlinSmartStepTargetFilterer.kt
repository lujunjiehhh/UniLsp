...
class KotlinSmartStepTargetFilterer(
    ...
    private suspend fun KotlinMethodSmartStepTarget.matches(
        owner: String,
        name: String,
        signature: String,
        currentCount: Int
        ...
        val matchingByNameAndOrdinalTargets =
            targets.filter { it.ordinal == currentCount && methodNameMatches(it.methodInfo, name) }
    ...
    private fun KaSession.primaryConstructorMatches(
        declaration: KtClass,
        owner: String,
        name: String,
        signature: String
...
private fun BytecodeSignature.handleDefaultArgs(): BytecodeSignature {
    ...
    if (isSyntheticDefaultMethodPossiblyConvertedToStatic(isStatic, withTailParamsRemoved.signature, typeSignature)) {
        return withTailParamsRemoved.copy(
            signature = buildSignature(
                withTailParamsRemoved.signature,
                1,
                fromStart = true
            )
        )
