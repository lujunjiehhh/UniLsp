...
internal fun matchesDefaultMethodSignature(
    defaultType: Type,
    actualType: Type,
    possiblyConvertedToStatic: Boolean,
    ...
    return actualArgTypes.matchesDefaultSignature(defaultArgTypes, isConstructor)
            || possiblyConvertedToStatic && actualArgTypes.matchesDefaultSignature(
        defaultArgTypes.drop(1).toTypedArray(), isConstructor
    )
}
...
internal fun isSyntheticDefaultMethodPossiblyConvertedToStatic(location: Location): Boolean {
    ...
    return isSyntheticDefaultMethodPossiblyConvertedToStatic(
        method.isStatic,
        method.signature(),
        location.declaringType().signature()
    )
}

internal fun isSyntheticDefaultMethodPossiblyConvertedToStatic(
    isStatic: Boolean,
    signature: String,
