...
class KotlinExceptionFilter(private val searchScope: GlobalSearchScope) : ExceptionFilter(searchScope) {
            ...
            if (!defaultLinkFileNames.contains(fileName)) {
                ...
                if (filesByName.isNotEmpty()) {
                    return if (filesByName.size > 1) {
                        HyperlinkInfoFactoryImpl.getInstance()
                            .createMultipleFilesHyperlinkInfo(filesByName.toList(), lineNumber, project)
                    } else {
    ...
    private fun createHyperlinks(
        jvmName: JvmClassName,
        file: VirtualFile,
        line: Int,
        project: Project
    ): InlineFunctionHyperLinkInfo? {
        ...
        val inlineFunCallInfo =
            mapStacktraceLineToSource(smapData, line, project, SourceLineKind.CALL_LINE, searchScope)
