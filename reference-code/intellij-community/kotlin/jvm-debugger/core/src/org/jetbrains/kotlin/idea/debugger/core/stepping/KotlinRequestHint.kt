...
class KotlinStepOutRequestHint(
    ...
    override fun getNextStepDepth(context: SuspendContextImpl): Int {
        ...
        if (currentLocation != null && returnAfterSuspendLocation != null) {
            ...
            thisLogger().debug {
                "KotlinStepOutRequestHint: stepping to the suspend RETURN in method ${
...
private fun addBreakpointAtFirstDeclaredLocationInLambda(
    hint: KotlinRequestHint,
    ...
    val filter = object : BreakpointStepMethodFilter {
        override fun locationMatches(process: DebugProcessImpl?, location: Location?) =
            location == firstDeclaredLocation
    ...
    val breakpoint = DebuggerManagerEx.getInstanceEx(context.debugProcess.project).getBreakpointManager()
        .addStepIntoBreakpoint(filter) ?: return false
