...
class KotlinFieldBreakpoint(
    ...
    override fun reload() {
        ...
        if (callable is KtProperty && callable.isTopLevel) {
            properties.myClassName =
                JvmFileClassUtil.getFileClassInfoNoResolve(callable.getContainingKtFile()).fileClassFqName.asString()
        } else {
    ...
    override fun getEvaluationElement(): KtCallableDeclaration? {
        return when (val callable =
            PositionUtil.getPsiElementAt(project, KtValVarKeywordOwner::class.java, sourcePosition)) {
