...
class ClassNameProvider(private val configuration: Configuration = Configuration.DEFAULT) {
    ...
    data class Configuration internal constructor(
        val findInlineUseSites: Boolean = false,
        val alwaysReturnLambdaParentClass: Boolean
    ) {
        companion object {
    ...
    interface ClassNameContributor {
        ...
        companion object {
            internal val EP_NAME =
                ExtensionPointName<ClassNameContributor>("org.jetbrains.kotlin.idea.debugger.core.classNameContributor")
        }
    }
    ...
    fun getCandidatesForElement(element: PsiElement): List<String> =
        getCandidatesForElementInternal(element).map { it.name }
    ...
    private fun computeCandidatesForElement(element: PsiElement): List<ClassNameCandidateInfo> {
        ...
        fun registerClassName(element: KtElement): String? {
             ...
             */
            @Suppress("DEPRECATION_ERROR")
            return org.jetbrains.kotlin.idea.debugger.base.util.ClassNameCalculator.getClassName(element)
                ?.also { registerClassName(it) }
        }
