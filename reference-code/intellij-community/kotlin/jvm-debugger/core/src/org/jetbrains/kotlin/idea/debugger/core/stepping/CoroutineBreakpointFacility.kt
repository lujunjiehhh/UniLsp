...
object CoroutineBreakpointFacility {
    fun installResumeBreakpointInCurrentMethod(suspendContext: SuspendContextImpl): Boolean {
        ...
        val resumeLocation =
            if (methodLineLocations.size > 2 && methodLineLocations[methodLineLocations.size - 2] == currentLocation) {
    ...
    fun installResumeBreakpointInCallerMethod(suspendContext: SuspendContextImpl): Boolean {
        ...
        val nextLocationAfterResume = getLocationOfNextInstructionAfterResume(resumeLocation)
        thisLogger().debug {
            "Trying to set a resume breakpoint in the caller method: " +
                    "resumeMethod: ${resumeLocation.safeMethod()}, " +
                    "nextCallLocationLine = ${nextLocationAfterResume?.lineNumber()}"
        }
        return installCoroutineResumedBreakpoint(suspendContext, resumeLocation, nextLocationAfterResume)
    ...
    private fun installCoroutineResumedBreakpoint(
        context: SuspendContextImpl,
        resumedLocation: Location,
        nextLocationAfterResume: Location?
    ): Boolean {
        ...
        if (debugProcess.requestsManager.filterThread == null) {
            thisLogger().error(
                "Coroutine filter should be calculated and set before breakpoint request created. " +
                        "In other case, this breakpoint may be hit while intermediate evaluations on other threads before the filter will be set."
            )
        }

        val breakpoint = object :
            StepIntoMethodBreakpoint(method.declaringType().name(), method.name(), method.signature(), project) {
            ...
            private fun scheduleStepOverCommandForSuspendSwitch(
                it: SuspendContextImpl,
                nextLocationAfterResume: Location?
            ): Boolean {
                DebuggerSteppingHelper.createStepOverCommandForSuspendSwitch(it, nextLocationAfterResume)
                    .prepareSteppingRequestsAndHints(it)
    ...
    private fun installBreakpointToRemoveSteppingInCurrentThread(context: SuspendContextImpl): ClearSteppingBreakpoint? {
        ...
        val debugProbesImpl =
            context.debugProcess.findLoadedClass(
                context,
    ...
    private fun installAdditionalBreakpoint(
        probeMethod: Method,
        additionalRequestor: AdditionalProbeBreakpoint,
        context: SuspendContextImpl
    ) {
...
private abstract class AdditionalProbeBreakpoint(project: Project) : SyntheticLineBreakpoint(project),
    SteppingBreakpoint {
...
private class ClearSteppingBreakpoint(project: Project, private val originalThread: ThreadReference) :
    AdditionalProbeBreakpoint(project) {
    var counter = 1
