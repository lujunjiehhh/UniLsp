        ...
        markerLocalVariables
            ...
            .any { variableName ->
                if (variableName.startsWith("-")) {
                    ...
                    ClassNameProvider(STOP_AT_LAMBDA).getCandidatesForElement(inlineArgument)
                        .any { lambdaClassNameCandidate ->
                            ...
                            candidateMethodName == functionName || nameMatchesUpToDollar(
                                candidateMethodName,
                                functionName
                            )
                        }
...
private fun checkContinuationLabelField(
    location: Location,
    name: String?,
    descriptor: String?,
    owner: String?
...
private class CoroutineStateMachineVisitor(method: Method, private val resumeLocation: Location) : MethodNode(
    Opcodes.ASM9,
    Opcodes.ACC_PUBLIC,
    method.name(),
    "",
    ...
    override fun visitMethodInsn(
        ...
        when {
            coroutineSuspendedState == CoroutineSuspendedState.NONE && isGetCoroutineSuspended(
                name,
                owner
            ) -> coroutineSuspendedState = CoroutineSuspendedState.INVOKE_GET_COROUTINE_SUSPENDED

            myState == CurrentInsn.NONE && reachedResumedLocation() -> myState = CurrentInsn.SUSPEND_METHOD_ARGS
            myState == CurrentInsn.SUSPEND_METHOD_ARGS && !isSuspendFunction(
                name,
                descriptor
            ) -> {
            } // skip function arguments
            myState == CurrentInsn.PUTFIELD_CONTINUATION_LABEL && isSuspendFunction(name, descriptor) -> myState =
                CurrentInsn.INVOKE_SUSPEND_METHOD
    ...
    override fun visitInsn(opcode: Int) {
        ...
        when {
            ...
            myState == CurrentInsn.ALOAD_SUSPEND_RESULT_FOR_SUSPEND_RETURN && opcode == Opcodes.ARETURN -> firstReturnAfterSuspensionOffset =
    ...
    override fun visitVarInsn(opcode: Int, `var`: Int) {
        ...
        when {
            coroutineSuspendedState == CoroutineSuspendedState.INVOKE_GET_COROUTINE_SUSPENDED && opcode == Opcodes.ASTORE -> coroutineSuspendedLocalVarIndex =
                `var`

            myState == CurrentInsn.NONE && reachedResumedLocation() -> myState = CurrentInsn.SUSPEND_METHOD_ARGS
            myState == CurrentInsn.SUSPEND_METHOD_ARGS -> {} // skip function arguments
            myState == CurrentInsn.DUP && opcode == Opcodes.ALOAD && `var` == coroutineSuspendedLocalVarIndex -> myState =
                CurrentInsn.ALOAD_SUSPEND_RESULT_BEFORE_COMPARE

            myState == CurrentInsn.IF_ACMPNE && opcode == Opcodes.ALOAD && `var` == coroutineSuspendedLocalVarIndex -> myState =
                CurrentInsn.ALOAD_SUSPEND_RESULT_FOR_SUSPEND_RETURN
    ...
    override fun visitFieldInsn(opcode: Int, owner: String?, name: String?, descriptor: String?) {
        ...
        when {
            myState == CurrentInsn.NONE && reachedResumedLocation() -> myState = CurrentInsn.SUSPEND_METHOD_ARGS
            myState == CurrentInsn.SUSPEND_METHOD_ARGS && opcode == Opcodes.PUTFIELD && checkContinuationLabelField(
                resumeLocation,
                name,
                descriptor,
                owner
            ) -> myState = CurrentInsn.PUTFIELD_CONTINUATION_LABEL
