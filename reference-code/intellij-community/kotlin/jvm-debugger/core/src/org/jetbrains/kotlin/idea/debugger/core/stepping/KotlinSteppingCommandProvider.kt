...
class KotlinSteppingCommandProvider : JvmSteppingCommandProvider() {
    ...
    fun getStepOutCommand(
        suspendContext: SuspendContextImpl,
        debugContext: DebuggerContextImpl
    ): DebugProcessImpl.ResumeCommand? {
...
fun getStepOverAction(
    ...
    val srcLocationToken =
        LocationToken.from(srcStackFrame).takeIf { it.lineNumber > 0 } ?: return KotlinStepAction.JvmStepOver
    ...
    val filter = object : KotlinStepOverFilter(location) {
        override fun isAcceptable(
            ...
            dstLocationToken: LocationToken,
            dstStackFrame: StackFrame
        ): Boolean {
            ...
            fun isInInlineFunctionBody(): Boolean {
                ...
                val commonPrefixLen =
                    (srcStackFrames zip dstStackFrames).count { (a, b) -> a.scopeVariable == b.scopeVariable }
...
private fun isInlineFunctionFromLibrary(
