...
class SmartStepTargetVisitor(
     ...
     */
    private fun findDelegatedMethod(
        delegate: KtPropertyDelegate,
        targetType: KtNameReferenceExpressionUsage?
    ): KtFunction? {
        val reference = delegate.mainReference ?: return null
        val expectedMethodName =
            if (targetType == KtNameReferenceExpressionUsage.PROPERTY_SETTER) "setValue" else "getValue"
    ...
    private fun KaSession.propertyAccessLabel(
        symbol: KaPropertySymbol,
        propertyAccessSymbol: KaDeclarationSymbol
    ): String {
    ...
    private fun KaSession.createKotlinLambdaInfo(
        ...
        return if (argumentSymbol.returnType.isFunctionalInterface) {
            ...
            KotlinLambdaInfo(
                ...
                isSam = true,
                isSamSuspendMethod = funMethodSymbol.isSuspend,
                methodName = funMethodSymbol.name.asString()
            )
        } else {
    ...
    private fun countExistingMethodCalls(declaration: KtDeclaration): Int =
        consumer.targetsWithDeclaration(declaration).count()
        ...
        if (checkLineRangeFits(expression.getLineRange())) {
            analyze(expression) {
                val variableAccessCall =
                    expression.resolveToCall()?.successfulCallOrNull<KaVariableAccessCall>() ?: return
