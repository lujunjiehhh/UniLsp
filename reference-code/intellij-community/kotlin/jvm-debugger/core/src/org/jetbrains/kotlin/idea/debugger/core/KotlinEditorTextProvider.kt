...
@TestOnly
@ApiStatus.Internal
fun <T> KotlinEditorTextProvider.Companion.withCustomConfiguration(
    useAnalysisApi: Boolean,
    block: (KotlinEditorTextProvider) -> T
): T {
...
private object AnalysisApiBasedKotlinEditorTextProvider : KotlinEditorTextProvider {
    ...
    private tailrec fun calculateCandidate(originalElement: PsiElement?, allowMethodCalls: Boolean): PsiElement? {
        ...
        return when (candidate) {
            ...
            is KtStringTemplateExpression -> if (isStringTemplateAllowed(
                    candidate,
                    allowMethodCalls
                )
            ) candidate else null
    ...
    private fun isReferenceAllowed(reference: KtReferenceExpression, allowMethodCalls: Boolean): Boolean =
        runDumbAnalyze(reference, fallback = false) f@{
            when {
