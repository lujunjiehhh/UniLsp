...
class KotlinLambdaAsyncMethodFilter(
                ...
                if (isSameCoroutineSuspendLambda) {
                    ...
                    if (filterThread != null) {
                        val breakpointManager =
                            DebuggerManagerEx.getInstanceEx(context.debugProcess.project).breakpointManager
    ...
    private inner class AsyncSuspendLambdaBreakpoint(
        ...
        lambdaMethod: Method
    ) : StepIntoMethodBreakpoint(
        lambdaMethod.declaringType().name(),
        lambdaMethod.name(),
        lambdaMethod.signature(),
                        ...
                        val hint: RequestHint =
                            object : RequestHint(
                                ...
                                override fun getNextStepDepth(context: SuspendContextImpl): Int {
                                    ...
                                    if (location != null && location.method().name() == "<init>" && location.method()
                                            .declaringType() == lambdaReference.referenceType()
                                    ) {
                                        ...
                                        val invokeSuspendMethod = lambdaReferenceCopy.referenceType().methods()
                                            .single { it.name() == INVOKE_SUSPEND }
                                        val breakpoint = AsyncSuspendLambdaBreakpoint(
                                            context,
                                            hint,
                                            lambdaReferenceCopy,
                                            invokeSuspendMethod
                                        )
                                        DebugProcessImpl.prepareAndSetSteppingBreakpoint(
                                            context,
                                            breakpoint,
                                            hint,
                                            true
