...
class KotlinSourcePositionHighlighter : SourcePositionHighlighter() {
    override fun getHighlightRange(sourcePosition: SourcePosition?): TextRange? {
        ...
        val lambda =
            element.parentOfType<KtFunction>()?.takeIf { it is KtFunctionLiteral || it.name == null } ?: return null
        ...
        if (sourcePosition.line == lambdaLineRange.first) {
            // Try to highlight only the body of lambda, otherwise take the whole lambda (including curly braces).
            val onlyBody =
                if (bodyRange.startOffset in intersection) intersection.cutInit(bodyRange.startOffset) else intersection
        ...
        if (sourcePosition.line == lambdaLineRange.last
            ...
            // Try to highlight only the body of lambda, otherwise take the whole lambda (including curly braces).
            val onlyBody =
                if (bodyRange.endOffset in intersection) intersection.cutTail(bodyRange.endOffset) else intersection
            return onlyBody
        }
