...
object DebuggerUtils {
    ...
    internal fun findSourceFilesForClass(
        ...
        if (DumbService.isDumb(project)
            && FileBasedIndex.getInstance().currentDumbModeAccessType != DumbModeAccessType.RELIABLE_DATA_ONLY
        ) return emptyList()
    ...
    @RequiresReadLock
    @ApiStatus.Internal
    fun getLocationsOfInlinedLine(
        type: ReferenceType,
        position: SourcePosition,
        sourceSearchScope: GlobalSearchScope
    ): List<Location> {
    ...
    private fun inlinedLinesNumbers(
        ...
        val file =
            findSourceFileForClassIncludeLibrarySources(project, sourceSearchScope, jvmClassName, destinationFileName)
                ?: return listOf()

        val virtualFile = file.virtualFile ?: return listOf()

        val sourceMap =
            KotlinSourceMapCache.getInstance(project).getSourceMap(virtualFile, jvmClassName) ?: return listOf()
