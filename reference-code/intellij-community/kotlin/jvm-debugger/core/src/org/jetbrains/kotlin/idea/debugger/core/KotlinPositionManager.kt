...
class KotlinPositionManager(private val debugProcess: DebugProcess) : MultiRequestPositionManager,
    ...
    override suspend fun createStackFramesAsync(descriptor: StackFrameDescriptorImpl): List<XStackFrame>? {
        ...
        val coroutineFrames =
            StackFrameInterceptor.instance?.createStackFrames(frameProxy, descriptor.debugProcess as DebugProcessImpl)
    ...
    override suspend fun getSourcePositionAsync(location: Location?): SourcePosition? {
        ...
        if (sourcePosition !is KotlinReentrantSourcePosition && location.shouldBeTreatedAsReentrantSourcePosition(
                psiFile,
                fileName
            )
        ) {
    ...
    private suspend fun Location.shouldBeTreatedAsReentrantSourcePosition(
        psiFile: PsiFile,
    ...
    private suspend fun getAlternativeSource(location: Location): PsiFile? {
        ...
        return readAction {
            val alternativeFileUrl =
                DebuggerUtilsEx.getAlternativeSourceUrl(qName, debugProcess.project) ?: return@readAction null
            val alternativePsiFile =
                VirtualFileManager.getInstance().findFileByUrl(alternativeFileUrl) ?: return@readAction null
    ...
    private suspend fun getCallableReferenceIfInside(
        location: Location,
        file: KtFile,
        lineNumber: Int
    ): KtCallableReferenceExpression? {
        val currentLocationClassName = location.getClassName() ?: return null
        val allReferenceExpressions =
            readAction { getElementsAtLineIfAny<KtCallableReferenceExpression>(file, lineNumber) }
    ...
    private suspend fun getLambdaOrFunOnLineIfInside(location: Location, file: KtFile, lineNumber: Int): KtFunction? {
        ...
        val (innermostContainingLiteral, notInlinedLambdas) = literalsOrFunctions.separateInlinedAndNonInlinedElements(
            location
        )
    ...
    private suspend fun List<KtFunction>.getAppropriateLiteralBasedOnLambdaName(
        location: Location,
        lineNumber: Int
    ): KtFunction? {
    ...
    @RequiresReadLock
    private fun getKotlinClassPrepareRequests(
        requestor: ClassPrepareRequestor,
        position: SourcePosition
    ): List<PrepareRequest> {
        ...
        candidates.flatMap { (name, _) -> listOf(name, "$name$*") }
            .mapTo(classRequests) { PrepareRequest(requestor, it) }
...
private data class PrepareRequest(
    val requestor: ClassPrepareRequestor,
    val pattern: String?,
    val fileName: String? = null
)

private fun List<ClassNameProvider.ClassNameCandidateInfo>.filterHasInlineElements() =
    filter { it.hasInlineElements }.map { it.name }
...
private fun fallbackHasInlinedLinesTo(
    referenceType: ReferenceType,
    line: Int,
    sourceCandidatesInternalName: List<String>
): Boolean {
