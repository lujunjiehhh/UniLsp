...
abstract class KotlinDescriptorTestCaseWithStepping : KotlinDescriptorTestCase() {
    companion object {
        //language=RegExp
        const val BAZEL_DEPENDENCY_LABEL_REGEX =
            """(classes|sources)\((@[a-zA-Z0-9_\-]+//([a-z0-9_\-]+)?:[a-z0-9._\-]+)\)"""
    }
    ...
    private fun initContexts(suspendContext: SuspendContextImpl) {
        ...
        myCommandProvider =
            JvmSteppingCommandProvider.EP_NAME.extensions.firstIsInstance<KotlinSteppingCommandProvider>()
    }
    ...
    private fun SuspendContextImpl.doRunToCursor(lineIndex: Int, fileName: String) {
        val runToCursorCommand = runReadAction {
            ...
            commandProvider.getRunToCursorCommand(this, xSourcePosition, false) ?: dp.createRunToCursorCommand(
                this,
                xSourcePosition,
                false
            )
    ...
    private fun process(instruction: SteppingInstruction) {
        ...
        when (instruction.kind) {
            SteppingInstructionKind.RunToCursor -> doOnBreakpoint {
                doRunToCursor(
                    instruction.lineIndex,
    ...
    private fun createSmartStepIntoFilters(suspendContext: SuspendContextImpl): List<MethodFilter> {
        ...
        if (runReadAction { stepTargets.none { DebugProcessImpl.isClassFiltered(it.className) } }) {
            try {
                assertEquals(
                    "Smart step targets are not sorted by position in tree",
        ...
        return runReadAction {
            stepTargets.mapNotNull { stepTarget ->
                when (stepTarget) {
                    ...
                    is MethodSmartStepTarget -> BasicStepMethodFilter(
                        stepTarget.method,
    ...
    private fun List<SmartStepTarget>.sortedByPositionInTree(): List<SmartStepTarget> {
        ...
        runReadAction {
            val elementAt = debuggerContext.sourcePosition.elementAt
                ?: error("Can not sort smart targets source position element is not defined")
            val searchEntryPoint = elementAt.getContainingMethod()
                ?: error("Can not sort smart targets as cannot find the containing element")
    ...
    override fun addDependenciesByLabels(
    ...
    protected data class BazelDependencyLabelDescriptor(val type: OrderRootType, val label: String) {
        companion object {
            fun fromString(text: String): BazelDependencyLabelDescriptor {
                ...
                val result =
                    regex.matchEntire(text) ?: error("Unable to parse '$text' in // ATTACH_LABEL: specification")
