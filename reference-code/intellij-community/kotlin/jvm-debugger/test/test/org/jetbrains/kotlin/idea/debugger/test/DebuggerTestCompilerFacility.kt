...
open class DebuggerTestCompilerFacility(
    ...
    fun compileTestSourcesWithCli(
        ...
        if (kotlinJvm.isNotEmpty() || kotlinCommon.isNotEmpty()) {
            ...
            doWriteAction {
                compileKotlinFilesWithCliCompiler(
                    jvmSrcDir,
                    commonSrcDir,
                    classesDir,
                    libClassesDir,
    ...
    protected open fun getCompileOptionsForMainSources(
        jvmSrcDir: File,
        commonSrcDir: File,
        moduleName: String
    ): List<String> {
    ...
    private fun createPsiFilesAndCollectKtFiles(testFiles: List<TestFile>, srcDir: File): List<KtFile> {
        ...
        for (file in testFiles) {
            ...
            val virtualFile =
                ioFile.refreshAndToVirtualFile() ?: error("Cannot find a VirtualFile instance for file $file")
...
internal fun File.refreshAndToVirtualFile(): VirtualFile? =
