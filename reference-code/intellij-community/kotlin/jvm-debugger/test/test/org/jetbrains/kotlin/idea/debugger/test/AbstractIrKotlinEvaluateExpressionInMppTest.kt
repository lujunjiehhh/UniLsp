...
abstract class AbstractIrKotlinEvaluateExpressionInMppTest : AbstractIrKotlinEvaluateExpressionTest() {
    ...
    private fun configureModuleByTestFiles(
        ...
        when (module) {
            is DebuggerTestModule.Common -> createAndConfigureCommonModule(
                context,
                module,
                platformName,
     ...
     */
    private fun configureLanguageFeatures(
        module: DebuggerTestModule,
        files: List<TestFileWithModule>,
        context: ConfigurationContext
    ) {
        val facet = (KotlinFacetSettingsProvider.getInstance(project) ?: error("Missing 'KotlinFacetSettingsProvider'"))
            .getSettings(context.workspaceModuleMap[module] ?: error("Missing 'module': ${module.name}"))
            ?: error("Missing facet")
    ...
    private fun handleDirectives(
        ...
        val platform =
            findAtMostOneDirectiveInModuleFiles(moduleFiles, PLATFORM_DIRECTIVE.toPrefix())?.trim()?.uppercase()?.also {
...
open class MppDebuggerCompilerFacility(
    ...
    override fun getCompileOptionsForMainSources(
        jvmSrcDir: File,
        commonSrcDir: File,
        moduleName: String
    ): List<String> {
