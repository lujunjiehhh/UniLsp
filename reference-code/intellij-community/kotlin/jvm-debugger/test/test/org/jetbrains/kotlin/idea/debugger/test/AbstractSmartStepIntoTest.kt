    ...
    protected open fun doTest(path: String) = runBlocking {
        ...
        val actual = withContext(Dispatchers.Default) {
            ...
            readAction {
                targets.map { target ->
                    val suffix =
                        if (target is KotlinMethodSmartStepTarget && targets.targetsWithDeclaration(target.getDeclaration())
                                .count() > 1
                        ) {
        ...
        val expected =
            InTextDirectivesUtils.findListWithPrefixes(fixture.file?.text!!.replace("\\,", "+++"), "// EXISTS: ")
                .map { it.replace("+++", ",") }
