...
class KotlinDebuggerEvaluatorTest : JVMDebuggerEvaluatorTest() {
    ...
    fun testVariable() {
        ...
        doTestRangeExpression(
            "val a = <caret>5",
            noExpressions()
        ) // just a constant expression, considered not interesting
    ...
    fun testClassNamePure() {
        doTestRange(
            "class Abc { fun foo() { val s = A<caret>bc::class.java.name } }",
            expressionWithoutSideEffects("Abc::class")
        )
        doTestRange(
            "class Abc { fun foo() { val s = Abc::cl<caret>ass.java.name } }",
            expressionWithoutSideEffects("Abc::class")
        )
    }
    ...
    fun testClassName1() {
        doTestRange(
            "class Abc { fun foo() { val b = A<caret>bc.aa } companion object { var aa: Int }}",
            expressionWithoutSideEffects("Abc")
        )
        doTestRange(
            "class Abc { fun foo() { val b = Abc.a<caret>a } companion object { var aa: Int }}",
            expressionWithoutSideEffects("Abc.aa")
        )
    }

    fun testClassName2() {
        doTestRange(
            "class Abc { fun foo() { A<caret>bc.aa(5) } companion object { fun aa(a: Int) {} }}",
            expressionWithoutSideEffects("Abc")
        )
        doTestRange(
            "class Abc { fun foo() { Abc.a<caret>a(5) } companion object { fun aa(a: Int) {} }}",
            expressionWithSideEffects("Abc.aa(5)")
        )
    }

    fun testFields() {
        doTestRange(
            "class Abc { int aa; void foo() { Abc ins = new Abc(); i<caret>ns.aa;}}",
            expressionWithoutSideEffects("ins")
        )
        doTestRange(
            "class Abc { int aa; void foo() { Abc ins = new Abc(); ins.a<caret>a;}}",
            expressionWithoutSideEffects("ins.aa")
        )
    }

    fun testContextParameters() {
        doTestRange(
            "context(c<caret>tx: String) fun f1(arg: String): String = ctx + arg",
            expressionWithoutSideEffects("ctx")
        )
        doTestRange(
            "context(ctx: String) fun f1(arg: String): String = c<caret>tx + arg",
            expressionWithoutSideEffects("ctx")
        )
    }
    ...
    fun testTernaryOperator() {
        ...
        doTestRangeExpression(
            "val expr = true; val a = if (expr<caret>) 56 else 73",
            expressionWithSideEffects("if (expr) 56 else 73")
        )
        doTestRangeExpression(
            "val expr = true; val a = if (expr)<caret> 56 else 73",
            expressionWithSideEffects("if (expr) 56 else 73")
        ...
        doTestRangeExpression(
            "val expr = true; val a = if (expr) 5<caret>6 else 73",
            expressionWithSideEffects("if (expr) 56 else 73")
        )
        doTestRangeExpression(
            "val expr = true; val a = if (expr) 56<caret> else 73",
            expressionWithSideEffects("if (expr) 56 else 73")
        )
        doTestRangeExpression(
            "val expr = true; val a = if (expr) 56 <caret>else 73",
        ...
        doTestRangeExpression(
            "val expr = true; val a = if (expr) 56 else<caret> 73",
            expressionWithSideEffects("if (expr) 56 else 73")
        )
    }
    ...
    fun testTernaryOperatorWithMethods() {
        doTestRangeExpression(
            "val expr = false; val a = if (expr) 5<caret>6 else bar(56)",
            expressionWithSideEffects("if (expr) 56 else bar(56)")
        )
    }
    ...
    fun testTryWithMethods() {
        ...
        doTestRangeExpression(
            "t<caret>ry { bar(37) } finally {}",
            expressionWithSideEffects("try { bar(37) } finally {}")
        )
        doTestRangeExpression(
            "try { bar(37) }<caret> finally {}",
            expressionWithSideEffects("try { bar(37) } finally {}")
        )
        doTestRangeExpression(
            "try { bar(37) } <caret>finally {}",
    ...
    fun testWhenWithMethods() {
        doTestRangeExpression(
            "val x = 37; when (x) { 1<caret>0 -> bar(10); 20 -> bar(20) }",
            expressionWithSideEffects("when (x) { 10 -> bar(10); 20 -> bar(20) }")
        )
    }
    ...
    fun testArrayAccessVar() {
        doTestRangeExpression(
            "val id = 5; val abc = Array<Int>(200); a<caret>bc[id]",
            expressionWithoutSideEffects("abc")
        )
        doTestRangeExpression(
            "val id = 5; val abc = Array<Int>(200); abc[i<caret>d]",
            expressionWithoutSideEffects("id")
        )
        doTestRangeExpression(
            "val id = 5; val abc = Array<Int>(200); abc<caret>[id]",
            expressionWithSideEffects("abc[id]")
        )
    }
    ...
    fun testCast() {
        ...
        doTestRangeExpression(
            "(::bar a<caret>s (Int) -> Int)(25)",
            expressionWithoutSideEffects("::bar as (Int) -> Int")
        )
    }
    ...
    private fun doTestRangeExpression(code: String, expected: ExpectedExpression) {
        doTestRange(
            "class A { fun foo() {" + code + "} fun bar(x: Int) = 37 fun isOdd(x: Int) = (x % 2 != 0) }",
            expected
        )
    }
