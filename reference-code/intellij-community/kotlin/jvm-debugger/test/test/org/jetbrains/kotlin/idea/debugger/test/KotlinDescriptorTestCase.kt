...
abstract class KotlinDescriptorTestCase : DescriptorTestCase(),
    ...
    protected open fun fileName(): String =
        getTestDataFileName(this::class.java, this.name) ?: (getTestName(false) + ".kt")
    ...
    private fun createAndConfigureJvmModule(
        ...
        val newWorkspaceModule =
            createModule(debuggerTestModule.name).also { context.workspaceModuleMap[debuggerTestModule] = it }
    ...
    private fun testModuleSrcPath(debuggerTestModule: DebuggerTestModule): String {
        val commonModuleSrcPath =
            listOf(testAppPath, MULTI_MODULES, debuggerTestModule.name).joinToString(File.separator)
    ...
    open fun doTest(unused: String) {
        ...
        val jvmDefaultMode =
            if (rawJvmDefaultMode == DebuggerPreferenceKeys.JVM_DEFAULT_MODE.defaultValue)
                ...
                JvmDefaultMode.fromStringOrNull(rawJvmDefaultMode)
                    ?: error("Invalid JVM default mode value: $rawJvmDefaultMode")
    ...
    private fun compileTestProjectSimple(
        ...
        updateIdeCompilerSettingsForEvaluator(
            languageVersion,
            enabledLanguageFeatures,
            compilerFacility.getCompilerPlugins()
        )
    ...
    private fun compileTestModule(
        ...
        val fileWithModules =
            context.filesByModules[debuggerTestModule] ?: error("Files for ${debuggerTestModule.name} not found")
        val module =
            context.workspaceModuleMap[debuggerTestModule] ?: error("Module ${debuggerTestModule.name} not found")
        ...
        val sourcesKtFilesForModule =
            compilerFacility.creatKtFiles(jvmSrcDir, commonSourcesOutputDirectory, scriptSourcesOutputDirectory)
    ...
    private fun compileLibrariesAndTestSources(
        ...
        sourcesKtFiles =
            compilerFacility.creatKtFiles(
                jvmSourcesOutputDirectory,
                commonSourcesOutputDirectory,
                scriptSourcesOutputDirectory
            )
    ...
    protected open fun getMainClassName(compilerFacility: DebuggerTestCompilerFacility): String {
        return getMainClassNameOrNull(compilerFacility, sourcesKtFiles.jvmKtFiles)
            ?: error("Cannot find a 'main()' function")
    }

    private fun getMainClassNameOrNull(
        compilerFacility: DebuggerTestCompilerFacility,
        jvmKtFiles: List<KtFile>
    ): String? {
        ...
        return runReadAction {
            ...
            when (candidates.size) {
                ...
                1 -> {
                    ...
                    val packagePrefix =
                        if (candidate.packageFqName.isRoot) "" else candidate.packageFqName.asString() + "."
    ...
    override fun createLocalProcess(transport: Int, javaParameters: JavaParameters): DebuggerSession? {
        ...
        debuggerRunnerSettings.debugPort =
            if (transport == DebuggerSettings.SOCKET_TRANSPORT) "0" else DEFAULT_ADDRESS.toString()
    ...
    override fun createBreakpoints(file: PsiFile?) {
        if (file != null) {
            val breakpointCreator =
                this.breakpointCreator ?: error(BreakpointCreator::class.java.simpleName + " should be set")
        ...
        runWriteAction {
            ...
            try {
                attachLibrary(
                    ...
                    listOf(
                        TestKotlinArtifacts.kotlinStdlib.toFile(),
                        TestKotlinArtifacts.jetbrainsAnnotations.toFile()
                    ),
                    listOf(
                        TestKotlinArtifacts.kotlinStdlibSources.toFile(),
                        TestKotlinArtifacts.kotlinStdlibCommonSources.toFile()
                    )
                )
...
class TestFiles(val originalFile: File, val wholeFile: TestFile, files: List<TestFileWithModule>) :
    List<TestFileWithModule> by files

sealed class DebuggerTestModule(name: String, dependencies: List<String>) :
    KotlinBaseTest.TestModule(name, dependencies, emptyList()) {
