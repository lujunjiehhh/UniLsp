...
internal class KotlinxCoroutinesCoreJvmJarRegexTest {
    ...
    fun doesNotMatchOtherArtifacts() {
        assertNull(
            regex.matchEntire("/repo/.m2/kotlin-stdlib-1.9.24.jar"),
            "Should not match other artifacts like kotlin-stdlib"
        )
        assertNull(
            regex.matchEntire("/repo/.m2/kotlinx-coroutines-core-common-1.7.3.jar"),
            "Should not match core-common variant"
        )
    }
    ...
    fun doesNotMatchMalformedJarFiles() {
        ...
        assertNull(
            regex.matchEntire("/repo/.m2/kotlinx-coroutines-core.1.7.3.jar"),
            "Should not match with '.' before version"
        )
    }
