    ...
    override fun doTest(
        ...
        val expectedRanks: Map<Pair<KtFile, Int>, Int> = files.asSequence().flatMap { ktFile ->
            ktFile.text.lines()
                ...
                .map {
                    val matchResult =
                        "^.*// (R: (-?\\d+)( L: (\\d+))?)\\s*$".toRegex().matchEntire(it.value) ?: return@map null
        ...
        for (outputFile in outputFiles) {
            ...
            val expectedFile =
                classNameToKtFile[className] ?: throw IllegalStateException("Can't find source for $className")
    ...
    runReadAction {
        buildMap {
            for (sourceFile in sourceFiles) {
                val classNames =
                    InTextDirectivesUtils.findListWithPrefixes(sourceFile.text, PRODUCED_CLASS_NAMES_DIRECTIVE)
