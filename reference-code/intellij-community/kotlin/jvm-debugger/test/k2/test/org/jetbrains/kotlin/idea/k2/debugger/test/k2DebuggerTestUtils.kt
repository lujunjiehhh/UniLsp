...
internal inline fun <R> withTestServicesNeededForCodeCompilation(project: Project, action: () -> R): R {
    ...
    val services = listOf(
        ...
        ServiceWithImplementation(
            KotlinModificationTrackerProvider::class.java,
            ::KotlinModificationTrackerProviderImpl
        ),
    )
    ...
    extensionArea.registerExtensionPoint(
        ResolveOptimizingOptionsProvider.EP_NAME.name,
...
class DummyResolutionAnchorProvider : ResolutionAnchorProvider {
    override fun getResolutionAnchor(moduleDescriptor: ModuleDescriptor): ModuleDescriptor? = null
}

internal data class ServiceWithImplementation<T : Any>(
    val serviceInterface: Class<T>,
    val createServiceInstance: (Project) -> T
)
