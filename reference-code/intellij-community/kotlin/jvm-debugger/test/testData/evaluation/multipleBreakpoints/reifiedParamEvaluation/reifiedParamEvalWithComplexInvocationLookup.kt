...
inline fun <reified T2> foo(x: String): String {
    return someFun() + someInlineFun() + bar<T2, Int>(x + "_bar<Int>") + someFun() + someInlineFun() + bar<T2, String>(x + "_bar<String>") + someFun() + someInlineFun() + bar<T2, Any>(
        x + "_bar<Any>"
    ) + someFun() + someInlineFun()
}

fun main() {
    someFun() + someInlineFun() + foo<Int>("foo<Int>") + someFun() + someInlineFun() + foo<String>(
        "foo<String>" + foo<Double>(
            "foo<Double>"
        )
    ) + someFun() + someInlineFun() + foo<Any>("foo<Any>") + someFun() + someInlineFun()
}