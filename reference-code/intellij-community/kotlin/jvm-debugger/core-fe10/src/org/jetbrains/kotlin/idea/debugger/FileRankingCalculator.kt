...
abstract class FileRankingCalculator(private val checkClassFqName: Boolean = true) : KotlinFileSelector {
    ...
    private suspend fun rankingForClass(
    ...
    private suspend fun rankingForMethod(function: KtFunction, method: Method): Ranking {
        ...
        val descriptor = bindingContext[BindingContext.DECLARATION_TO_DESCRIPTOR, function] as? CallableMemberDescriptor
            ?: return ZERO
    ...
    private suspend fun rankingForVisibility(
    ...
    private suspend fun fileRanking(file: KtFile, location: Location): Ranking {
        ...
        if (method.isIndyLambda() || method.isAnonymousClassLambda()) {
            ...
            if (method.isBridge && method.isSynthetic) {
                ...
                val containingCallable =
                    readAction { findNonLocalCallableParent(containingFunctionLiteral) } ?: return LOW
        ...
        if (method.name() == "<init>") {
            ...
            val constructorOrInitializer = readAction {
                elementAt.getParentOfTypes2<KtConstructor<*>, KtClassInitializer>()
                    ?.takeIf { containingClass.isAncestor(it) }
    ...
    private tailrec fun findNonLocalCallableParent(element: PsiElement): PsiElement? {
        fun PsiElement.isCallableDeclaration() =
            this is KtProperty || this is KtFunction || this is KtAnonymousInitializer
