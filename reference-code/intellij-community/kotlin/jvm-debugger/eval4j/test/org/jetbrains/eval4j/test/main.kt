...
fun initFrame(
    ...
    if ((m.access and ACC_STATIC) == 0) {
        ...
        val thisValue =
            REFLECTION_EVAL.invokeMethod(newInstance, MethodDescription(owner, "<init>", "()V", false), listOf(), true)
...
object REFLECTION_EVAL : Eval {
    ...
    override fun newMultiDimensionalArray(arrayType: Type, dimensionSizes: List<Int>): Value {
        return ObjectValue(
            ArrayHelper.newMultiArray(findClass(arrayType.elementType), *dimensionSizes.toTypedArray()),
            arrayType
        )
    }
    ...
    override fun getArrayElement(array: Value, index: Value): Value {
        ...
        val elementType =
            if (asmType.dimensions == 1) asmType.elementType else Type.getType(asmType.descriptor.substring(1))
    ...
    fun findClass(memberDesc: MemberDescription): Class<Any> =
        findClass(Type.getObjectType(memberDesc.ownerInternalName))
