...
private val BOOTSTRAP_CLASS_DESCRIPTORS =
    setOf(STRING_TYPE.descriptor, Type.getDescriptor(ClassLoader::class.java), CLASS_TYPE.descriptor)
...
open class JDIEval(
    ...
    private fun findMethod(
        ...
        clazz: ReferenceType = methodDesc.ownerType.asReferenceType()
    ): Method {
    ...
    open fun jdiInvokeStaticMethod(
        type: ClassType,
        method: Method,
        args: List<jdi_Value?>,
        invokePolicy: Int
        ...
        val result = mayThrow {
            when (declaringType) {
                ...
                is InterfaceType -> {
                    if (!isJava8OrLater) {
                        val message =
                            "Calling interface static methods is not supported in JVM ${vm.version()} ($method)"
    ...
    private fun invokeMethodWithReflection(
        ownerType: Type,
        instance: Value,
        args: List<jdi_Value?>,
        methodDesc: MethodDescription
