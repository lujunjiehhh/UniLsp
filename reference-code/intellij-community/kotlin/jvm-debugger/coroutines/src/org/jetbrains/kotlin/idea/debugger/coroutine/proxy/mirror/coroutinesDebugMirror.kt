...
class DebugProbesImpl private constructor(context: DefaultExecutionContext) :
    ...
    private val dumpCoroutinesInfoAsJsonAndReferences by MethodDelegate<ArrayReference>(
    ...
    private fun isInstalled(context: DefaultExecutionContext): Boolean =
        isInstalledInDebugMethod.value(instance, context)?.booleanValue() ?: isInstalledInCoreMethod.value(
            instance,
            context
        )?.booleanValue() ?: false
...
class DebugCoroutineInfoImpl(context: DefaultExecutionContext) :
    BaseMirror<ObjectReference, MirrorOfCoroutineInfo>(
    ...
    private val getCreationStackTrace by MethodMirrorDelegate(
        "getCreationStackTrace",
        JavaUtilAbstractCollection(context)
    )
...
class DebugCoroutineInfo(context: DefaultExecutionContext) :
    BaseMirror<ObjectReference, MirrorOfDebugCoroutineInfo>(
    ...
    private val lastObservedStackTrace by MethodMirrorDelegate(
    ...
    override fun fetchMirror(value: ObjectReference, context: DefaultExecutionContext): MirrorOfDebugCoroutineInfo {
        return MirrorOfDebugCoroutineInfo(
            ...
            creationStackTrace.mirror(value, context)?.values?.mapNotNull { stackTraceElement.mirror(it, context) }
                ?: emptyList(),
            lastObservedStackTrace.mirror(value, context)?.values?.mapNotNull { stackTraceElement.mirror(it, context) }
                ?: emptyList()
        )
    }
}
...
class CoroutineInfo private constructor(
    ...
    private val creationStackTraceMethod by MethodMirrorDelegate(
        "getCreationStackTrace",
        JavaUtilAbstractCollection(context)
    )
    ...
    companion object {
        ...
        fun instance(context: DefaultExecutionContext): CoroutineInfo? {
            val classType =
                context.findClassSafe(AGENT_135_AND_UP_CLASS_NAME) ?: context.findClassSafe(AGENT_134_CLASS_NAME)
                ?: return null
            return try {
