...
internal fun fetchCoroutineStacksInfoData(
...
private fun fetchContinuationStack(
    ...
    val (continuationStackFrames, creationStack) = collectCoroutineAndCreationStack(continuation, context)
        ?: return null
...
private fun parseResultFromHelper(
    array: Value,
    context: DefaultExecutionContext
): Pair<List<CoroutineStackFrameItem>, List<StackTraceElement>?>? {
...
private fun fallbackToOldFetchContinuationStack(
    ...
    val continuationStack =
        DebugMetadata.instance(context)?.fetchContinuationStack(continuation, context) ?: return null
