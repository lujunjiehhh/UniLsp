...
internal class CoroutineView(project: Project, javaDebugProcess: JavaDebugProcess) :
    ...
    fun collapseCoroutineHierarchyNode() {
        ...
        DebuggerUIUtil.invokeLater {
            panel.tree.root?.let { treeRoot ->
                ...
                if ((rootNode as? XValueNodeImpl)?.name == KotlinDebuggerCoroutinesBundle.message("coroutine.view.node.jobs") && !panel.tree.isCollapsed(
                        pathToRootNode
                    )
                ) {
            ...
            invokeInSuspendContext(suspendContext) { suspendContext ->
                ...
                for (dispatcher in groups.keys) {
                    ...
                    children.add(
                        DispatcherContainer(
                            suspendContext,
                            dispatcher ?: EMPTY_DISPATCHER_NAME,
                            isCurrent,
                            coroutines
                        )
                    )
    ...
    inner class CoroutineContainer(
        ...
        renderer.renderThreadGroup(
            rootCoroutineInfo?.coroutineDescriptor ?: rootJob ?: CoroutineInfoData.UNKNOWN_JOB, isCurrent
        )
    ) {
        override fun computeChildren(node: XCompositeNode) {
            invokeInSuspendContext(suspendContext) { suspendContext ->
    ...
    open inner class RendererContainer(val presentation: SimpleColoredTextIcon) :
        XNamedValue(presentation.simpleString()) {
        override fun computePresentation(node: XValueNode, place: XValuePlace): Unit =
