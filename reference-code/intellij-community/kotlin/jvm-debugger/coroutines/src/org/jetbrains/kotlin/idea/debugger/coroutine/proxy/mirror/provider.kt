...
class MethodMirrorDelegate<T, F>(
    val name: String,
    private val mirrorProvider: MirrorProvider<T, F>,
    val signature: String? = null
) : ReadOnlyProperty<ReferenceTypeProvider, MethodEvaluator.MirrorMethodEvaluator<T, F>> {
    override fun getValue(
        thisRef: ReferenceTypeProvider,
        property: KProperty<*>
    ): MethodEvaluator.MirrorMethodEvaluator<T, F> {
        return MethodEvaluator.MirrorMethodEvaluator(
            DebuggerUtils.findMethod(thisRef.getCls(), name, signature),
            mirrorProvider
        )
    }
}

class MethodDelegate<T>(val name: String, val signature: String? = null) :
    ReadOnlyProperty<ReferenceTypeProvider, MethodEvaluator<T>> {
...
class FieldMirrorDelegate<T, F>(
    val name: String,
    private val mirrorProvider: MirrorProvider<T, F>
) : ReadOnlyProperty<ReferenceTypeProvider, FieldEvaluator.MirrorFieldEvaluator<T, F>> {
    override fun getValue(
        thisRef: ReferenceTypeProvider,
        property: KProperty<*>
    ): FieldEvaluator.MirrorFieldEvaluator<T, F> {
        return FieldEvaluator.MirrorFieldEvaluator(
            DebuggerUtils.findField(thisRef.getCls(), name),
            thisRef,
            mirrorProvider
        )
    }
