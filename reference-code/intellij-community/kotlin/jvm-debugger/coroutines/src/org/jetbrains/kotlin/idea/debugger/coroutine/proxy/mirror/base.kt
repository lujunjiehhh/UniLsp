...
abstract class BaseMirror<T : ObjectReference, F>(val name: String, context: DefaultExecutionContext) :
    ReferenceTypeProvider, MirrorProvider<T, F> {
    val log by logger
    private val cls =
        context.findReferenceTypeSafe(name) ?: throw IllegalStateException("coroutine-debugger: class $name not found.")
