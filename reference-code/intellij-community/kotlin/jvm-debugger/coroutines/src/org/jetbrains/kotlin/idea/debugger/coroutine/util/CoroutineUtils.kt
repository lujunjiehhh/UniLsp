...
const val CREATION_STACK_TRACE_SEPARATOR =
    "\b\b\b" // the "\b\b\b" is used as creation stacktrace separator in kotlinx.coroutines
    ...
    return when (suspendExitMode) {
        SuspendExitMode.SUSPEND_LAMBDA -> {
            ...
            if (prevStackFrame == null) {
                logger.thisLogger().error(
                    "[coroutine filtering]: Could not extract the previous stack frame for the frame ${frameProxy.stackFrame}:\n" +
