...
private class CoroutineStackFrameInterceptor : StackFrameInterceptor {
    override suspend fun createStackFrames(
    ...
    private fun extractContinuationId(
        ...
        while (true) {
            ...
            if (continuationMirror.coroutineOwner != null) {
                ...
                val coroutineOwnerMirror =
                    coroutineOwner.mirror(continuationMirror.coroutineOwner, defaultExecutionContext)
    ...
    private fun getContinuationFilterFromHelper(
        ...
        val continuationIdValue = callMethodFromHelper(
            CoroutinesDebugHelper::class.java,
            context,
            "tryGetContinuationId",
            listOf(currentContinuation)
        )
    ...
    private fun getCoroutinesRunningOnCurrentThreadFromHelper(
        ...
        val result = callMethodFromHelper(
            CoroutinesDebugHelper::class.java,
            context,
            "getCoroutinesRunningOnCurrentThread",
            args
        )
        result ?: return null
    ...
    override fun callerLocation(suspendContext: SuspendContextImpl): Location? {
        ...
        val callerFrame = callMethodFromHelper(
            CoroutinesDebugHelper::class.java,
            executionContext,
            "getCallerFrame",
            listOf(continuationObject)
        ) ?: return null
        val stackTraceElement =
            debugMetadata.getStackTraceElement(callerFrame as ObjectReference, executionContext)?.stackTraceElement()
                ?: return null
        return stackTraceElement.let {
            DebuggerUtilsEx.findOrCreateLocation(
                suspendContext.virtualMachineProxy.virtualMachine,
                it
            )
        }
    }
    ...
    private data class CoroutineIdFilter(val coroutinesRunningOnCurrentThread: Set<Long>) : CoroutineFilter {
        ...
        override fun canRunTo(nextCoroutineFilter: CoroutineFilter): Boolean =
            (nextCoroutineFilter is CoroutineIdFilter && coroutinesRunningOnCurrentThread.intersect(nextCoroutineFilter.coroutinesRunningOnCurrentThread)
                .isNotEmpty())

        override val coroutineFilterName: String
            get() {
...
internal fun callMethodFromHelper(
    ...
    try {
        ...
        DebuggerUtilsImpl.logError(
            "Exception from helper: ${e.message}", e,
            *listOfNotNull(helperExceptionStackTrace).toTypedArray()
