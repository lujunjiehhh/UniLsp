...
internal fun createHotSwapSessionListenerOrNull(session: DebuggerSession): HotSwapStatusListener? {
    ...
    val hotSwapSession =
        HotSwapDebugSessionManager.getInstance(session.project).findHotSwapSession(process) ?: return null
    return HotSwapStatusListenerAdapter(session, hotSwapSession.startHotSwapListening())
}

private class HotSwapStatusListenerAdapter(
    private val originalSession: DebuggerSession,
    private val listener: HotSwapResultListener
) : HotSwapStatusListener {
    ...
    private fun List<DebuggerSession>.createListeners(except: DebuggerSession) = filter { it !== except }
        .mapNotNull {
            HotSwapDebugSessionManager.getInstance(except.project).findHotSwapSession(it.xDebugSession!!.debugProcess)
        }
