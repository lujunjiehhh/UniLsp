...
object DebuggerStatistics : CounterUsagesCollector() {
     ...
     */
    private val breakpointTypeField = EventFields.String(
        "type", listOf(
    ...
    private val breakpointInstallationOverhead =
        GROUP.registerEvent("breakpoint.install.overhead", breakpointTypeField, EventFields.DurationMs)

    /** Reports overhead spent on searching for classes related to a breakpoint. */
    private val breakpointInstallSearchOverhead =
        GROUP.registerEvent("breakpoint.install.search.overhead", breakpointTypeField, EventFields.DurationMs)

    /** Reports overhead caused by debugger considering the necessity to stop on the breakpoint (condition breakpoints). */
    private val breakpointVisitOverhead =
        GROUP.registerEvent("breakpoint.visit.overhead", breakpointTypeField, EventFields.DurationMs)

    /** Reports overhead caused by debugger considering stepping procedure (smart step into, kotlin step out). */
    private val steppingOverhead =
        GROUP.registerEvent("stepping.overhead", steppingActionField, languageField, EventFields.DurationMs)

    /** Reports smart step into unexpected end. Could be caused by unexpected exception. */
    private val steppingFailedMethodNotCalled =
        GROUP.registerEvent("stepping.method.not.called", steppingActionField, languageField)

    /** Reports successful or failed targets detection in smart-step-into. */
    private val smartStepTargetsDetection = GROUP.registerEvent(
        "smart.step.into.targets.detected",
        languageField,
        EventFields.Enum<JvmSmartStepIntoHandler.SmartStepIntoDetectionStatus>("status")
    )

    private val breakpointSkipped = GROUP.registerEvent(
        "breakpoint.skipped",
        EventFields.Enum<DebugProcessEvents.SkippedBreakpointReason>("reason")
    )

    private val threadDump = GROUP.registerEvent(
        "thread.dump",
        EventFields.Enum<ThreadDumpStatus>("status"),
        dumpedCoroutinesCounter,
        dumpedVirtualThreadsCounter
    )

    /** Reports successful or failed attempts to get evaluatable context on pause **/
    private val evaluationOnPause =
        GROUP.registerEvent("evaluation.on.pause", EventFields.Enum<EvaluationOnPauseStatus>("status"))

    private val threadDumpTriggeringException =
        GROUP.registerEvent("thread.dump.triggering.exception", threadDumpTriggeringExceptionField, EventFields.Count)

    /** Reports execution time of debugger commands in buckets, updated at the end of a debugger session. */
    private val timeBucketCount = GROUP.registerEvent(
        "debugger.command.time.bucket.updated",
        EventFields.Int("bucket_upper_limit_ms"),
        EventFields.Count,
        EventFields.Boolean("is_remote")
    )
    internal val bucketUpperLimits =
        (generateSequence(1L) { it * 2 }.takeWhile { it <= 2048 } + Long.MAX_VALUE).toList().toLongArray()
    ...
    fun logProcessStatistics(debugProcess: DebugProcess) {
        ...
        val intUpperLimits =
            bucketUpperLimits.map { if (it == Long.MAX_VALUE) Int.MAX_VALUE else it.toInt() }.toIntArray()
    ...
    @JvmStatic
    fun logSmartStepIntoTargetsDetection(
        project: Project?,
        language: Engine,
        status: JvmSmartStepIntoHandler.SmartStepIntoDetectionStatus
    ) {
