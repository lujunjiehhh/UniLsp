...
class FileObjectRenderer : CompoundRendererProvider() {
    ...
    override fun getChildrenRenderer(): ChildrenRenderer =
        NodeRendererSettings.createExpressionChildrenRenderer("listFiles()", null)
    ...
    override fun getFullValueEvaluatorProvider(): FullValueEvaluatorProvider? {
        return object : FullValueEvaluatorProvider {
            override fun getFullValueEvaluator(
                ...
                try {
                    val isFile =
                        DebuggerUtilsImpl.invokeObjectMethod(evaluationContext, value, "isFile", "()Z", emptyList())
                    if ((isFile as? BooleanValue)?.value() == true) {
                        return object :
                            JavaValue.JavaFullValueEvaluator(message("message.node.open"), evaluationContext) {
                            override fun isShowValuePopup(): Boolean = false

                            override fun evaluate(callback: XFullValueEvaluationCallback) {
                                val path = DebuggerUtilsImpl.invokeObjectMethod(
                                    evaluationContext,
                                    value,
                                    "getAbsolutePath",
                                    "()Ljava/lang/String;",
                                ...
                                if (path is StringReference) {
                                    callback.evaluated("")
                                    val vFile =
                                        LocalFileSystem.getInstance().refreshAndFindFileByPath(path.value()) ?: return
                                    DebuggerUIUtil.invokeLater {
                                        PsiNavigationSupport.getInstance()
                                            .createNavigatable(evaluationContext.project, vFile, -1).navigate(true)
                                    }
                                }
                            }
                        }
