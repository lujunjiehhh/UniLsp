    ...
    override fun getFullValueEvaluatorProvider(): FullValueEvaluatorProvider {
        return object : FullValueEvaluatorProvider {
            override fun getFullValueEvaluator(
                ...
                return object :
                    JavaFullValueEvaluator(JavaDebuggerBundle.message("message.node.navigate"), evaluationContext) {
                    override fun isShowValuePopup(): Boolean = false

                    override fun evaluate(callback: XFullValueEvaluationCallback) {
                        try {
                            val res = DebuggerUtilsImpl.invokeObjectMethod(
                                evaluationContext,
                                value,
                                "toString",
                                "()Ljava/lang/String;",
                            ...
                            if (res is StringReference) {
                                ...
                                ReadAction.run<Throwable> {
                                    ...
                                    val filter =
                                        ExceptionFilter(project, evaluationContext.debugProcess.session.searchScope)
