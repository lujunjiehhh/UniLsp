...
internal class JvmSmartStepIntoActionHandler(private val session: DebuggerSession) :
    XSmartStepIntoHandler<JvmSmartStepIntoVariant>() {
    ...
    private fun findVariants(xPosition: XSourcePosition, smart: Boolean): Promise<List<JvmSmartStepIntoVariant>> {
        val scope =
            (session.xDebugSession as XDebugSessionImpl).currentSuspendCoroutineScope ?: return rejectedPromise()
        ...
        return scope.future {
            val targetsPromise =
                if (smart) handler.findSmartStepTargetsAsync(position, session) else handler.findStepIntoTargets(
                    position,
                    session
                )
    ...
    internal class JvmSmartStepIntoVariant(
        ...
        override fun needForceSmartStepInto(): Boolean =
            target is ForceSmartStepIntoSource && target.needForceSmartStepInto()
    }
}
