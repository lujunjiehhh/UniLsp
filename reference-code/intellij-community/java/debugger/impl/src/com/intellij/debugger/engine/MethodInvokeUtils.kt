...
object MethodInvokeUtils {
    ...
    fun getHelperExceptionStackTrace(evaluationContext: EvaluationContextImpl, e: Exception): String? {
        ...
        val values =
            DebuggerUtilsImpl.invokeThrowableGetStackTrace(exceptionFromTargetVM, evaluationContext, true)?.values
        ...
        val stackTraceString =
            getExceptionTextFromStackTraceValues(evaluationContext, exceptionFromTargetVM, values, keepLines)
    ...
    fun getMethodHandlesImplLookup(evaluationContext: EvaluationContextImpl): ObjectReference? {
        val theClass = evaluationContext.debugProcess.findClass(
            evaluationContext,
            "java.lang.invoke.MethodHandles\$Lookup",
            null
        )
        val theField = DebuggerUtils.findField(
            theClass,
            "IMPL_LOOKUP"
        )
...
private val ORIGINS_FOR_USE_WITH_HELPER = EnumSet.of(
    XEvaluationOrigin.DIALOG,
...
internal fun tryInvokeWithHelper(
    ...
    invokerArgs.add(
        DebuggerUtilsEx.mirrorOfString(
    ...
    try {
        var value = DebuggerUtilsImpl.invokeHelperMethod(
            evaluationContext,
            MethodInvoker::class.java,
            helperMethodName,
            invokerArgs,
            false
        )
        ...
        if (helperExceptionStackTrace?.lineSequence()?.filterNot(::isHelperFrame)
                ?.any { it.contains(methodCallString) } == true
        ) {
            throw e
        }
        ...
        if (ApplicationManager.getApplication().isInternal) {
            val attachments = listOfNotNull(helperExceptionStackTrace?.let {
                Attachment("helper_stack.txt", it).apply {
                    isIncluded = true
                }
            }).toTypedArray()
            DebuggerUtilsImpl.logError(
                "Exception from helper (while evaluating ${methodDeclaringType.name() + "." + method.name()}): ${e.message}",
                RuntimeExceptionWithAttachments(e, *attachments)
            ) // log helper exception if available
        }
        return INVOCATION_FAILED
    }
}

private val HELPER_FRAMES =
    setOf(MethodInvoker::class.qualifiedName + ".invoke", "java.lang.invoke.MethodHandle.invoke")
