...
object DebuggerDiagnosticsUtil {
    ...
    fun checkThreadsConsistency(process: DebugProcessImpl, reportDiffWithRealCounter: Boolean) {
        ...
        for (threadProxy in allThreads) {
            ...
            val resumedByWatching =
                if (invocationWatching != null && suspendingContexts.contains(invocationWatching.mySuspendAllContext)) 1
                else 0
        ...
        if (problems.isNotEmpty()) {
            process.logError(
                "Found ${problems.size} problems",
                Attachment("Problems.txt", problems.joinToString(separator = "\n"))
            )
    ...
    private fun createStateAttachments(process: DebugProcessImpl): List<Attachment> {
        if (recursionTracker.get() == true) {
            return listOf(
                Attachment(
                    "Recursion_problem_detected_Just_dump.txt",
                    noErr { ThreadDumper.dumpThreadsToString() })
            )
        } else {
            try {
                ...
                return buildList {
                    ...
                    add(
                        Attachment(
                            "context_detailed_information.txt",
                            process.suspendManager.eventContexts.joinToString("\n") { it.toAttachmentString() })
                    )
                }
            } finally {
    ...
    @JvmStatic
    fun getEventSetClasses(eventSet: EventSet): String =
        "[${eventSet.map { it.javaClass.simpleName }.joinToString(", ")}]"
}
