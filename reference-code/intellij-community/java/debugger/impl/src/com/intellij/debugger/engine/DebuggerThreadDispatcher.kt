...
internal class DebuggerThreadDispatcher(private val managerThread: DebuggerManagerThreadImpl) : CoroutineDispatcher() {
    ...
    private fun createCommand(context: CoroutineContext, block: Runnable): DebuggerCommandImpl {
        val provider = context[DebuggerDispatchedCommandProvider.Key]
            ?: error("No ${DebuggerDispatchedCommandProvider.Key} in context")
...
internal fun Dispatchers.Debugger(managerThread: DebuggerManagerThreadImpl): CoroutineDispatcher =
    managerThread.debuggerThreadDispatcher

/**
 * Debugger command factory passed to the [DebuggerThreadDispatcher] for correct command scheduling.
 */
internal sealed class DebuggerDispatchedCommandProvider(val priority: PrioritizedTask.Priority) :
...
internal class DebuggerCommandProvider(priority: PrioritizedTask.Priority) :
