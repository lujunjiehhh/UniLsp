...
internal class SteppingProgressTracker(private val debuggerProcessImpl: DebugProcessImpl) {
    ...
    fun addStepping(
        stepCompetedStatus: CompletableDeferred<Unit>,
        isSuspendAllStepping: Boolean,
        filter: LightOrRealThreadInfo?
    ) {
...
internal class CancelingSteppingListener : SteppingListener {
    override fun beforeSteppingStarted(suspendContext: SuspendContextImpl, steppingAction: SteppingAction) {
        ...
        val needSuspendOnlyThread =
            suspendContext.suspendPolicy == EventRequest.SUSPEND_EVENT_THREAD && threadForStepping != null
        ...
        managerThread.schedule(PrioritizedTask.Priority.NORMAL) {
            ...
            whereStrFuture.thenAccept { whereStr ->
                ...
                managerThread.makeCancelable(
                    debuggerProcessImpl.project,
                    steppingRestrictionMessage,
                    steppingName,
                    stepCompetedStatus
                ) {
...
private val SteppingAction.steppingName: @Nls String
    get() = when (this) {
...
private fun getSteppingRestrictionMessage(
    whereSteppingIsPerformed: @Nls String?,
