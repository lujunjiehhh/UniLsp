...
class CollectionHistoryView(
    private val myClsName: String,
    ...
    private val myHistoryInstancesTree: InstancesTree = InstancesTree(
        myDebugProcess.project, myDebugSession.debugProcess.editorsProvider,
        getValueMarkers(myDebugProcess)
    ) { }
    private val myHistoryTree: InstancesTree = InstancesTree(
        myDebugProcess.project, myDebugSession.debugProcess.editorsProvider,
        getValueMarkers(myDebugProcess)
    ) { }
    ...
    private fun setupInstancesTree() {
        myHistoryInstancesTree.addTreeSelectionListener(TreeSelectionListener {
            ...
            invokeInDebuggerThread {
                ...
                val clsType =
                    (node.valueContainer as? JavaValue)?.descriptor?.type?.name() ?: return@invokeInDebuggerThread
    ...
    private fun setupHistoryTreeSelectionListener() {
        ...
        myHistoryTree.addTreeSelectionListener(TreeSelectionListener {
            ...
            val parentRow =
                myHistoryTree.root.children.withIndex().firstOrNull { pair -> pair.value == selectedNode }?.index
            ...
            invokeInDebuggerThread {
                ...
                val items = if (parentNode == myHistoryTree.root) {
                    ...
                    if (clsName == null) {
                        clsName =
                            (getSelectedNode(myHistoryInstancesTree)?.valueContainer as? JavaValue)?.descriptor?.type?.name()
                    }

                    val parent = myValueNode?.parent ?: getSelectedNode(myHistoryInstancesTree)
                    val field =
                        getObjectReferenceForNode(parent as? XValueNodeImpl)?.referenceType()?.fieldByName(myFieldName)
                            ?: return@invokeInDebuggerThread
                    val clsInstance =
                        if (!field.isStatic) getObjectReferenceForNode(parent as? XValueNodeImpl) else null
                    val modificationIndex = virtualMachineProxy.mirrorOf(parentRow!!)
                    CollectionBreakpointUtils.getFieldModificationStack(
                        mySuspendContext,
                        myFieldName,
                        clsName,
                        clsInstance,
                    ...
                    CollectionBreakpointUtils.getCollectionModificationStack(
                        mySuspendContext,
                        collectionInstance,
                        modificationIndex
                    )
    ...
    private fun loadFieldHistory(clsName: String, fieldName: String, parent: XValueNodeImpl) {
        ...
        val fieldModifications =
            CollectionBreakpointUtils.getFieldModificationsHistory(mySuspendContext, fieldName, clsName, clsInstance)
    ...
    private fun loadHistory() {
        myValueNode?.let { node ->
            invokeInDebuggerThread {
                val descriptor =
                    (node.valueContainer as? JavaValue)?.descriptor as? FieldDescriptor ?: return@invokeInDebuggerThread
        ...
        mySuspendContext.managerThread.invokeAndWait(object : DebuggerCommandImpl() {
            override fun action() {
                for (ins in values) {
                    ...
                    val value = InstanceJavaValue(
                        JavaReferenceInfo(ins as ObjectReference).createDescriptor(myDebugProcess.project), evalContext,
                        myNodeManager
                    )
                    if (renderer != null) {
    ...
    private inner class CollectionHistoryRenderer : ClassRenderer() {
        ...
        override fun buildChildren(value: Value?, builder: ChildrenBuilder?, evaluationContext: EvaluationContext?) {
            ...
            val collectionModifications =
                CollectionBreakpointUtils.getCollectionModificationsHistory(mySuspendContext, collectionInstance)

            val nodes = collectionModifications
                .filterIsInstance<ObjectReference>()
                .map {
                    val modificationInfo =
        ...
        private inner class ChildrenRenderer(val obj: ObjectReference?, val isAddition: Boolean?) : ClassRenderer() {
            ...
            private fun evaluate(evaluationContext: EvaluationContext): Pair<Value, Value>? {
                val cls = myDebugProcess.findClass(
                    evaluationContext, "com.intellij.rt.debugger.agent.CollectionBreakpointInstrumentor\$Pair",
                    null
