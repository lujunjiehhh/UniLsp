...
internal fun scheduleSourcePositionCompute(
    ...
    executeOnDMT(suspendContext, priority) {
        ...
        if (inline) {
            val inlinePosition =
                SourcePositionProvider.getSourcePosition(descriptor, descriptor.project, debugContext, true)
    ...
    return cs.future {
        ...
        withDebugContext(suspendContext) {
            ...
            if (value is ObjectReference) {
                objectReferenceInfo = JavaValueObjectReferenceInfo(
                    value.referenceType().name(),
                    value.virtualMachine().canGetInstanceInfo()
                )
            }
        }
