...
class DebuggerManagerThreadImpl @ApiStatus.Internal @JvmOverloads constructor(
    ...
    override fun invokeAndWait(managerCommand: DebuggerCommandImpl) {
        LOG.assertTrue(
            !isManagerThread(),
            "Should be invoked outside manager thread, use DebuggerManagerThreadImpl.schedule(...)"
        )
    ...
    @Deprecated(
        "Use invokeNow if in DebuggerManagerThread or schedule otherwise",
    ...
    companion object {
        ...
        fun assertIsManagerThread() {
            LOG.assertTrue(
                isManagerThread(),
                "Should be invoked in manager thread, use DebuggerManagerThreadImpl.schedule(...)"
            )
        }
...
fun executeOnDMT(
    ...
    context = Dispatchers.Debugger(suspendContext.managerThread) + SuspendContextCommandProvider(
        suspendContext,
        priority
    ),
...
suspend fun <T> withDebugContext(
    ...
    return runWithContext(
        context = Dispatchers.Debugger(managerThread) + provider, parentScope = scope,
        block = wrapVMDisconnectedException(block)
    )
}
