...
open class CompoundPositionManager() : PositionManagerWithConditionEvaluation, MultiRequestPositionManager,
    PositionManagerAsync {
    private val myPositionManagers = mutableListOf<PositionManager>()
    private val mySourcePositionCache: MutableMap<Location?, SourcePosition?> =
        WeakHashMap<Location?, SourcePosition?>()
    ...
    override fun getSourcePosition(location: Location?): SourcePosition? =
        getCachedSourcePosition(location, { action -> runReadAction(action) }) { fileType: FileType? ->
            ReadAction.nonBlocking<SourcePosition?> {
                iterate<SourcePosition?>(
                    null,
                    fileType,
                    ProgressManager::checkCanceled
                ) { it.getSourcePosition(location) }
...
private suspend fun getSourcePositionAsync(positionManager: PositionManager, location: Location?): SourcePosition? {
    ...
    try {
        return ReadAction.nonBlocking<SourcePosition?> { positionManager.getSourcePosition(location) }
            .executeSynchronously()
    } catch (e: Exception) {
...
private inline fun checkCacheEntry(
    position: SourcePosition,
    className: String,
    insideReadAction: (() -> Unit) -> Unit
): Boolean =
    callInReadAction(insideReadAction) {
        ...
        val url =
            DebuggerUtilsEx.getAlternativeSourceUrl(className, psiFile.getProject()) ?: return@callInReadAction true
