...
class ThreadDumpAction {
    companion object {
        ...
        @ApiStatus.Internal
        suspend fun buildThreadDump(
            context: DebuggerContextImpl,
            onlyPlatformThreads: Boolean,
            dumpItemsChannel: SendChannel<List<MergeableDumpItem>>
        ) {
            ...
            if (onlyPlatformThreads || !Registry.`is`("debugger.thread.dump.extended")) {
                ...
                DebuggerStatistics.logPlatformThreadDumpFallback(
                    context.project,
                    if (onlyPlatformThreads) ThreadDumpStatus.PLATFORM_DUMP_ALT_CLICK else ThreadDumpStatus.PLATFORM_DUMP_EXTENDED_DUMP_DISABLED
                )
                return
            }
            ...
            try {
                ...
                if (providers.any { it.requiresEvaluation }) {
                    ...
                    if (vm.getUserData(EVALUATION_IN_PROGRESS) == true) {
                        ...
                        DebuggerStatistics.logPlatformThreadDumpFallback(
                            context.project,
                            ThreadDumpStatus.PLATFORM_DUMP_FALLBACK_DURING_EVALUATION
                        )
                        XDebuggerManagerImpl.getNotificationGroup()
                            .createNotification(
                                JavaDebuggerBundle.message("thread.dump.during.previous.dump.evaluation.warning"),
                    ...
                    try {
                        ...
                        dumpItemsChannel.send(
                            ...
                            InfoDumpItem(
                                JavaDebuggerBundle.message("thread.dump.unavailable.title"),
                            "Timeout while waiting for evaluatable context, unable to dump ${
                        ...
                        DebuggerStatistics.logPlatformThreadDumpFallback(
                            context.project,
                            ThreadDumpStatus.PLATFORM_DUMP_FALLBACK_TIMEOUT
                        )
                    } finally {
                ...
                when (e) {
                    is CancellationException, is ControlFlowException -> throw e
                        ...
                        DebuggerStatistics.logPlatformThreadDumpFallback(
                            context.project,
                            ThreadDumpStatus.PLATFORM_DUMP_FALLBACK_ERROR
                        )
                    }
                }
            }
...
private inline fun <reified T : Type> findThreadFieldImpl(
...
private inline fun <reified T : Type> findThreadField(
    fieldNames: List<String>,
    jlThreadType: ReferenceType?,
    fieldHolderType: ReferenceType?,
...
private fun buildThreadStates(
    ...
    fun getFieldValue(field: Field?, threadReference: ThreadReference, fieldHolder: ObjectReference?): Value? {
        ...
        return when (val fieldHost = field.declaringType()) {
            ...
            fieldHolderType -> fieldHolder!!.getValue(field)
                ...
                logger<ThreadDumpAction>().error("unexpected declaring type of field $field: $fieldHost"); null
            }
        }
    }
    ...
    fun processOne(threadReference: ThreadReference, virtualThreadInfo: Pair<String, Long>?) {
        ...
        if (virtualThreadInfo != null) {
            ...
            isDaemon =
                getFieldValue(daemonField, threadReference, holderObj)?.let { (it as BooleanValue).booleanValue() }
                    ?: false
        ...
        val collectMonitorsInfo = virtualThreadInfo == null ||
                virtualThreads.size < Registry.intValue(
            "debugger.thread.dump.virtual.threads.with.monitors.max.count",
            1000
        )
        try {
