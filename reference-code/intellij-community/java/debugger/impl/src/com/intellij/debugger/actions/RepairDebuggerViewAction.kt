    ...
    override fun actionPerformed(e: AnActionEvent) {
        ...
        process.managerThread.schedule(object : DebuggerCommandImpl() {
            override fun action() {
                ...
                val restoredDebuggerContext = DebuggerContextImpl.createDebuggerContext(
                    session,
                    suspendContextToRestore,
                    suspendContextToRestore.eventThread,
                    null
                )
                DebuggerInvocationUtil.invokeLater(process.project) {
                    session.contextManager.setState(
                        restoredDebuggerContext,
                        DebuggerSession.State.PAUSED,
                        DebuggerSession.Event.PAUSE,
                        null
                    )
                }
            }
