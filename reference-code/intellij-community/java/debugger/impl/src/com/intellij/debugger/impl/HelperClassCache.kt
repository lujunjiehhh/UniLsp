...
internal class HelperClassCache(debugProcess: DebugProcessImpl, managerThread: DebuggerManagerThreadImpl) {
    ...
    fun getHelperClass(
        ...
        try {
            ...
            return when (classLoaderInfo) {
                ...
                is DefinedInCompanionClassLoader? -> tryLoadingInCompanion(
                    classLoaderInfo,
                    evaluationContext,
                    className,
    ...
    private fun tryLoadingInParentOrCompanion(
        ...
        try {
            if (currentClassLoader != null) {
                ...
                val type = tryToDefineInClassLoader(
                    evaluationContext,
                    parentClassLoader,
                    currentClassLoader,
                    className,
    ...
    private fun tryLoadingInCurrent(
        ...
        val currentClassLoader = evaluationContext.classLoader
        val loadedTypes = listOf(className, *additionalClassesToLoad).map {
            tryLoadInClassLoader(
                evaluationContext,
                it,
                currentClassLoader
            )
    ...
    private fun tryLoadingInCompanion(
        ...
        try {
            ...
            val type = tryToDefineInClassLoader(
                evaluationContext, companionClassLoader, companionClassLoader,
                className, *additionalClassesToLoad
            ) ?: return null
            if (currentInfo == null) {
                DebuggerUtilsAsync.disableCollection(companionClassLoader)
                evaluationClassLoaderMapping[evaluationContext.classLoader] =
                    DefinedInCompanionClassLoader(companionClassLoader)
            }
            return type
        } catch (e: Throwable) {
...
private fun tryLoadInClassLoader(
    evaluationContext: EvaluationContextImpl,
    className: String,
        ...
        rtJarClassLoader.getResourceAsStream("${name.replace('.', '/')}.class").use { stream ->
            ...
            try {
                ClassLoadingUtils.defineClass(
                    ...
                    stream.readAllBytes(),
                    evaluationContext,
                    evaluationContext.debugProcess,
                    classLoader
                )
            } catch (e: EvaluateException) {
