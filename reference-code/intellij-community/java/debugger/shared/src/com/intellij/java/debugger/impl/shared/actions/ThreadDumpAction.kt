...
internal class ThreadDumpAction : DumbAwareAction(), SplitDebuggerAction {
    ...
    override fun actionPerformed(e: AnActionEvent) {
        ...
        sessionProxy.coroutineScope.launch {
            ...
            val maxItems =
                if (dtosWillBeSerialized) Registry.intValue("debugger.thread.dump.max.items.frontend") else Int.MAX_VALUE
            val (threadDumpsDtoChannel, filters) = JavaDebuggerSessionApi.getInstance()
                .dumpThreads(sessionProxy.id, maxItems, onlyPlatformThreads) ?: return@launch
...
private suspend fun collectAndShowDumpItems(
    ...
    for (dump in threadDumpsChannel) {
        threadDumpPanel.addDumpItems(
            dump.threadDump, dump.threadDumpTruncatedSize,
...
private fun ThreadDumpWithAwaitingDependencies.toDumpItems(): List<DumpItem> {
    ...
    val feDumpItems =
        items.map { FrontendDumpItem(it, iconsCache, attributesCache, stackTraces, stateDescriptions, iconToolTips) }
...
private class FrontendDumpItem(
    ...
    override val attributes: SimpleTextAttributes
        get() = attributesCache[itemDto.attributesIndex.toInt().toUInt().toInt()]
